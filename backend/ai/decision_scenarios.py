...
class ScenarioEngine:
    ...
    async def handle_scenario_1_high_risk_booking(
        self, booking: Dict[str, Any], attendee_info: Dict[str, Any]
    ) -> ScenarioResult:
        """
...
Please ensure team is prepared and premium suite is reserved.
    ...
    async def handle_scenario_3_timezone_conflict(
        ...
        meeting_time_utc = datetime.fromisoformat(
...
Best regards,
    ...
    async def handle_scenario_4_conflict_detected(
        ...
        decision = await self.decision_engine.analyze_and_decide(
            booking=booking,
            attendee_info=attendee_info,
            context={"calendar_conflict": True, "conflict_event": conflict_event},
        )
...
Please choose an alternative time or resolve the conflict manually.
        ...
        decision.human_review_reason = (
            "Calendar conflict detected - human intervention required"
        )
...
Calendar invite attached.
        ...
        return ScenarioResult(
            ...
            special_handling=["Standard automation", "Minimal overhead"],
    ...
    async def handle_scenario_6_follow_up_booking(
        self, booking: Dict[str, Any], attendee_info: Dict[str, Any], memory_manager
    ) -> ScenarioResult:
        """
        ...
        if not memory_manager:
            logger.warning(
                "Memory manager unavailable for follow-up booking; using defaults."
            )
            ...
            past_interaction = await memory_manager.retrieve_by_context(
                query=f"meeting with {booking['attendees'][0]}", n_results=1
            )
        ...
        if past_interaction:
            ...
            attendee_info["preferred_communication"] = learned_preferences.get(
                "preferred_communication", "email"
            )
            booking["location"] = learned_preferences.get(
                "preferred_location", "Conference Room A"
            )
            booking["duration_minutes"] = learned_preferences.get(
...
Calendar invite attached.
    ...
    def _format_timezone_table(self, conversions: List[Dict]) -> str:
        ...
        for conv in conversions:
            lines.append(
                f"{conv['attendee']} | {conv['timezone']} | {conv['local_time']}"
            )
        return "\n".join(lines)
    ...
    def _suggest_alternative_times(self, original_time: str) -> List[str]:
        ...
        alternatives.append(
            (original + timedelta(days=1, hours=2)).strftime("%Y-%m-%d %H:%M")
        )
...
async def run_all_scenarios():
    ...
    if engine.memory_manager is not None:
        scenarios.append(
            ("Follow-up Booking", engine.handle_scenario_6_follow_up_booking)
        )
    else:
        logger.warning(
            "Skipping scenario 6 follow-up booking because memory manager could not be initialized."
        )
