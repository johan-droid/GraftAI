...
class AuditLogger:
    ...
    async def log(
        ...
        if result == Result.FAILURE:
            ...
            await AuditLogger._send_security_alert(
                event_type, event_category, log_message, metadata
            )
    ...
    async def log_data_access(
        ...
        event_type_map = {
            Action.CREATE: EventType.USER_CREATE
            if resource_type == "user"
            else EventType.CALENDAR_CREATE,
            Action.READ: EventType.USER_VIEW
            if resource_type == "user"
            else EventType.CALENDAR_VIEW,
            Action.UPDATE: EventType.USER_UPDATE
            if resource_type == "user"
            else EventType.CALENDAR_UPDATE,
            Action.DELETE: EventType.USER_DELETE
            if resource_type == "user"
            else EventType.CALENDAR_DELETE,
        }
        ...
        return await AuditLogger.log(
            ...
            else EventCategory.DATA_MODIFICATION,
            action=action,
        ...
        try:
            ...
            alert_payload = {
                ...
                "blocks": [
                    ...
                    {
                        "type": "section",
                        "text": {"type": "mrkdwn", "text": f"```{log_message}```"},
                    },
                ],
            }
