...
async def create_zoom_meeting(db: AsyncSession, user_id: str, event_details: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not access_token:
            msg = "Zoom authentication failed - no valid token available."
        ...
        if "end_time" in event_details and "start_time" in event_details:
            try:
                ...
                pass
        payload = {"topic": event_details.get("title", "GraftAI Zoom Meeting"), "type": 2, "start_time": start_time_str, "duration": max(duration, 1), "settings": {"host_video": True, "participant_video": True, "join_before_host": True, "mute_upon_entry": True, "auto_recording": "none", "waiting_room": False}}
...
async def list_zoom_meetings(db: AsyncSession, user_id: str) -> list:
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
        client = await get_client()
        resp = await client.get("https://api.zoom.us/v2/users/me/meetings?type=upcoming&page_size=30", headers=headers)
