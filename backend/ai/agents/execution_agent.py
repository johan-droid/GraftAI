...
class ExecutionAgent(BaseAgent):
    ...
    async def _execute_single(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"Executing {action_type} (attempt {attempt}/{max_retries})")
    ...
    async def _action_create_calendar_event(self, params: Dict, user_id: str) -> Dict:
        ...
        from backend.ai.tools.scheduling_tools_real import create_calendar_event as create_calendar_event_real
    ...
    async def _rollback_calendar_event(self, params: Dict, user_id: str):
        ...
        if event_id:
            ...
            if not delete_result.get("success"):
                raise RuntimeError(delete_result.get("error", "Calendar event deletion failed"))
