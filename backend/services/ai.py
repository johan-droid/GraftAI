...
def _milestone_for_intent(intent: str, action: dict[str, Any] | None=None) -> str | None:
    if not intent:
        return None
    if action and action.get("status") in {"error", "conflict", "not_found"}:
        return None
    return {"list": "schedule_reviewed", "schedule": "meeting_scheduled", "update": "meeting_updated", "delete": "meeting_deleted"}.get(intent)
...
def _candidate_groq_models() -> list[str]:
    configured = [m.strip() for m in os.getenv("GROQ_MODELS", "llama-3.3-70b-versatile,llama-3.1-70b-versatile,llama-3.1-8b-instant").split(",") if m.strip()]
...
def _get_groq_client() -> Any:
    if AsyncGroq is None:
        msg = "Groq SDK not installed"
        raise RuntimeError(msg)
    api_key = os.getenv("GROQ_API_KEY")
    if not api_key:
        msg = "GROQ_API_KEY not configured"
        raise RuntimeError(msg)
...
async def _refresh_calendar_context_if_needed(db: AsyncSession, user_id: str) -> None:
    """
    Keep AI calendar context fresh without syncing on every request.
    This ensures the agent sees recent external Google/Microsoft updates.
    """
    freshness_key = f"ai:calendar_context_synced:{user_id}"
    if await get_cache(freshness_key):
        return
    token_stmt = select(UserTokenTable.id).where(and_(UserTokenTable.user_id == user_id, UserTokenTable.is_active, UserTokenTable.provider.in_(["google", "microsoft"])))
    has_active_calendar = (await db.execute(token_stmt)).scalars().first()
    if not has_active_calendar:
        await set_cache(freshness_key, "no_external_calendar", 120)
        return
    try:
        await sync_user_calendar(db, user_id)
    except Exception as exc:
        logger.warning("Calendar context pre-sync failed for AI chat user=%s: %s", user_id, exc)
    finally:
        await set_cache(freshness_key, "ok", 120)
...
def _extract_duration_minutes(prompt: str) -> int:
    lower = prompt.lower()
    match = re.search("(\\d+)\\s*(minute|minutes|min|mins|hour|hours|hr|hrs)", lower)
    if not match:
        return 30
...
def _extract_event_id(prompt: str) -> int | None:
    patterns = ["(?:event|id)\\s*#?\\s*(\\d+)", "\\b(\\d+)\\b"]
...
def _looks_like_meeting_request(prompt: str) -> bool:
    ...
    if any(provider in lower for provider in ["zoom", "teams", "microsoft", "google meet", "gmeet", "meet link"]):
        return True
    meeting_keywords = ["meeting", "call", "sync", "standup", "1:1", "one-on-one", "interview", "kickoff", "demo", "review", "conference"]
    return any(keyword in lower for keyword in meeting_keywords)

def _parse_update_action_payload(action_text: str, user_timezone: str) -> tuple[int | None, datetime | None]:
    match = re.search("ACTION:UPDATE_MEETING:(\\{.*\\})", action_text, flags=re.IGNORECASE)
    if not match:
        return (None, None)
    try:
    ...
    return (event_id_value, parsed.astimezone(UTC))
...
def _extract_json_payload(raw_text: str) -> dict[str, Any] | None:
    ...
    if not text:
        return None
    fenced = re.search("```(?:json)?\\s*(\\{.*?\\})\\s*```", text, flags=re.IGNORECASE | re.DOTALL)
    if fenced:
...
def _extract_title(prompt: str) -> str:
    quoted = re.search('[\'\\"]([^\'\\"]{3,120})[\'\\"]', prompt)
    if quoted:
        return quoted.group(1).strip()
    for_match = re.search("\\b(?:for|about|regarding)\\s+([^\\n\\r\\.,!\\?]+)", prompt, flags=re.IGNORECASE)
    if for_match:
        candidate = for_match.group(1).strip(" -:")
        candidate = re.sub("^(?:a|an|the)\\s+", "", candidate, flags=re.IGNORECASE).strip()
        if candidate:
            return candidate[:120].title()
    cleaned = re.sub("\\s+", " ", prompt).strip()
    cleaned = re.sub("\\b\\d{1,2}(?::\\d{2})?\\s*(?:am|pm)?\\b", "", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub("\\b(schedule|book|add|create|event|appointment|task|reminder|meeting|call|for|at|on|today|tomorrow|next|week|month|day|morning|afternoon|evening)\\b", "", cleaned, flags=re.IGNORECASE).strip(" -:")
    cleaned = re.sub("^(?:a|an|the)\\s+", "", cleaned, flags=re.IGNORECASE).strip()
    if cleaned:
...
def _extract_datetime(prompt: str, user_timezone: str) -> datetime:
    ...
    lower = prompt.lower()
    iso_match = re.search("(\\d{4}-\\d{2}-\\d{2}[t\\s]\\d{1,2}:\\d{2}(?::\\d{2})?)", prompt)
    if iso_match:
    ...
    meridian_match = re.search("(\\d{1,2})(?::(\\d{2}))?\\s*(am|pm)", lower)
    hour = None
    minute = 0
    if meridian_match:
        ...
        if meridian_match.group(3) == "pm":
            hour += 12
    if hour is None:
        hm_match = re.search("\\b(\\d{1,2}):(\\d{2})\\b", lower)
        if hm_match:
    ...
    if hour is None:
        ...
        start_local = datetime(target_day.year, target_day.month, target_day.day, hour, minute, tzinfo=tz)
...
async def _resolve_event_id(db: AsyncSession, user_id: str, prompt: str) -> int | None:
...
def _format_events(events: list[Any], user_timezone: str) -> str:
    ...
    for event in events[:8]:
        ...
        lines.append(f"- #{event_id} {title} | {start_local.strftime('%a %b %d, %I:%M %p')} - {end_local.strftime('%I:%M %p')}")
    if not lines:
...
async def _offline_assistant_response(prompt: str, user_timezone: str, db: AsyncSession, user_id: str) -> tuple[str, dict[str, Any]]:
    ...
    if intent == "list":
        ...
        return ("Here is your current schedule for the next 7 days:\n" + formatted, {"type": "list", "count": len(events)})
    if intent == "schedule":
        ...
        elif any(k in lower_prompt for k in ["google meet", "gmeet", "google-meet", "meet link"]) or re.search("\\b(?:on|via|using)\\s+meet\\b", lower_prompt):
            meeting_platform = "google"
        if is_meeting_request and (not meeting_platform):
            return ("I can schedule that. Which meeting platform should I use: Google Meet, Zoom, or Teams?", {"type": "clarify", "action": "select_platform"})
        start_time = _extract_datetime(prompt, user_timezone)
        duration = _extract_duration_minutes(prompt)
        agenda = None
        agenda_match = re.search("agenda[:\\s]+(.*?)(?:\\.|$)", prompt, flags=re.IGNORECASE)
        if agenda_match:
            agenda = agenda_match.group(1).strip()
        try:
            title_prompt = "Extract a concise event title. If no clear title is present, return a short sensible title. Output ONLY the title string."
            title = await _generate_with_groq(title_prompt, prompt)
            title = title.strip().strip('\'"')
        except Exception:
            title = _extract_title(prompt)
        event_data = {"user_id": user_id, "title": title, "description": f"Agenda: {agenda}" if agenda else "Created by GraftAI Sovereign Assistant", "start_time": start_time, "end_time": start_time + timedelta(minutes=duration), "source": "local", "fingerprint": f"ai-{start_time.timestamp()}", "is_meeting": bool(meeting_platform), "meeting_provider": meeting_platform, "metadata_payload": {"source": "assistant_offline_engine"}}
        conflicts = await scheduler.get_events_for_range(db, user_id, event_data["start_time"], event_data["end_time"])
        conflict_msg = ""
        if conflicts:
            conflict_names = ", ".join([f"'{_get_event_attr(c, 'title', 'event')}'" for c in conflicts[:2]])
        ...
        return (f"Done, I scheduled '{created.title}' for {start_label} ({user_timezone}) as event #{created.id}.{conflict_msg}", {"type": "schedule", "event_id": created.id, "start_time": created.start_time.isoformat(), "end_time": created.end_time.isoformat()})
    if intent == "update":
        event_id = await _resolve_event_id(db, user_id, prompt)
        new_start_override: datetime | None = None
        if not event_id or "to" in prompt.lower() or "at" in prompt.lower():
            action_hint = ""
            payload: dict[str, Any] | None = None
            try:
                system_instruction = "You are a precise data extraction engine. Extract calendar update parameters from the message. Output a strictly valid JSON object with: 'event_id' (int), 'new_start_time' (ISO-8601 string), 'duration' (int in minutes). If a value is missing, return null for that key. Return ONLY the JSON object."
                ...
                logger.warning("Provider extraction unavailable for update action: %s", e)
            if payload:
                try:
                    ...
                    if payload.get("new_start_time"):
                        ...
                        if parsed.tzinfo is None:
                            parsed = parsed.replace(tzinfo=_safe_zoneinfo(user_timezone))
                        new_start_override = parsed.astimezone(UTC)
                except (ValueError, TypeError) as e:
                    logger.warning("Structured update payload parse failed: %s", e)
        ...
        if not event_id:
            return ("I could not find an event to update. Mention an event id like 'update event 42 to tomorrow 3pm'.", {"type": "update", "status": "not_found"})
        new_start = new_start_override or _extract_datetime(prompt, user_timezone)
        duration = _extract_duration_minutes(prompt)
        update_payload = {"start_time": new_start, "end_time": new_start + timedelta(minutes=duration)}
        updated = await scheduler.update_event(db, event_id, user_id, update_payload)
        if not updated:
            return (f"Event #{event_id} was not found.", {"type": "update", "status": "not_found", "event_id": event_id})
        ...
        if isinstance(updated_start, datetime):
            if updated_start.tzinfo is None:
                updated_start = updated_start.replace(tzinfo=UTC)
            updated_time_label = updated_start.astimezone(UTC).strftime("%Y-%m-%d %H:%M UTC")
        else:
            updated_time_label = new_start.strftime("%Y-%m-%d %H:%M UTC")
        return (f"Updated event #{updated_id} to {updated_time_label}", {"type": "update", "event_id": updated_id})
    if intent == "delete":
        event_id = await _resolve_event_id(db, user_id, prompt)
        if not event_id:
            return ("I could not find an event to delete. Mention an event id like 'delete event 42'.", {"type": "delete", "status": "not_found"})
        deleted = await scheduler.delete_event(db, event_id, user_id)
        if not deleted:
            return (f"Event #{event_id} was not found.", {"type": "delete", "status": "not_found", "event_id": event_id})
        return (f"Deleted event #{event_id}.", {"type": "delete", "event_id": event_id})
    return ("I can help with calendar actions: list, schedule, update, or delete events. Try: 'schedule design sync tomorrow 3pm for 45 minutes'.", {"type": "none"})

async def _generate_with_groq_response(system_prompt: str, user_input: str, json_mode: bool=False) -> tuple[str, str]:
    ...
    for model_name in _candidate_groq_models():
        for attempt in range(1, max_retries + 1):
            kwargs: dict[str, Any] = {"model": model_name, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_input}], "temperature": 0.0 if json_mode else temperature}
            if json_mode:
            ...
            try:
                ...
                msg = "Empty response from Groq model"
                raise RuntimeError(msg)
            except Exception as exc:
                last_error = exc
                logger.warning("Groq request failed model=%s attempt=%s/%s json_mode=%s error=%s", model_name, attempt, max_retries, json_mode, type(exc).__name__)
    error_detail = f"{type(last_error).__name__}: {last_error}" if last_error else "unknown"
    msg = f"All configured Groq model attempts failed ({error_detail})"
    raise RuntimeError(msg)
...
async def _execute_ai_chat(payload: AIRequest, user_id: str, db: AsyncSession, _usage_check: bool) -> AIResponse:
    ...
    match intent:
        case "list" | "schedule" | "update" | "delete":
            try:
                result_text, action = await _offline_assistant_response(prompt, payload.timezone, db, user_id)
            except ValueError as ve:
                logger.warning("Offline action logic failure: %s", ve)
                result_text = f"⚠️ {ve!s}"
                action = {"type": intent, "status": "conflict"}
            except Exception as exc:
                logger.error("Offline system fail: %s", exc, exc_info=True)
            ...
            try:
                ...
                logger.warning("AI usage bookkeeping failed (offline): %s", exc, exc_info=True)
            return AIResponse(result=result_text, model_used="graftai-offline", action=action, milestone=_milestone_for_intent(intent, action))
        case _:
            pass
    ...
    if isinstance(cached, str) and cached.strip():
        try:
            ...
            logger.warning("AI usage bookkeeping failed (cache hit): %s", exc, exc_info=True)
        return AIResponse(result=cached, model_used="graftai-assistant-cache", action={"type": "none"}, milestone=None)
    now = datetime.now(UTC)
    await _refresh_calendar_context_if_needed(db, user_id)
    ...
    try:
        ...
        try:
            ...
            result_text = f"I'm in High-Stability mode due to a provider service outage. I've retrieved your agenda for the next 48 hours to keep you on track:\n\n{agenda}\n\nI can still perform specific actions like 'schedule', 'delete', or 'update' if you use clear commands."
        except Exception:
            result_text = "I'm currently in High-Stability mode due to a provider service outage. I can still see your local calendar data, but my natural language reasoning is temporarily restricted. How can I help with your existing schedule?"
    ...
    if not result_text:
        try:
            ...
            logger.warning("Local assistant engine ainvoke issue: %s", exc)
    if not result_text:
        result_text = "I am running in offline scheduling mode with the local assistant fallback. You can ask me to list, schedule, update, or delete calendar events."
    action = {"type": "none"}
    ...
    try:
        ...
        logger.warning("AI usage bookkeeping failed (online): %s", exc, exc_info=True)
    if milestone:
        try:
            await send_message(user_id, result_text, {"kind": "ai_milestone", "intent": intent, "milestone": milestone, "model_used": model_used})
        except Exception as exc:
            logger.warning("AI milestone stream publish failed: %s", exc, exc_info=True)
    return AIResponse(result=result_text, model_used=model_used, action=action, milestone=milestone)

@router.post("/legacy/chat", response_model=AIResponse)
@rate_limit(limit=10, window=60)
async def ai_chat(request: Request, payload: AIRequest, user_id: str=Depends(get_current_user_id), db: AsyncSession=Depends(get_db), _usage_check: bool=Depends(check_usage_limit("ai_messages"))):
    return await _execute_ai_chat(payload, user_id, db, _usage_check)

@router.post("/stream")
@rate_limit(limit=10, window=60)
async def ai_chat_stream(request: Request, payload: AIRequest, user_id: str=Depends(get_current_user_id), db: AsyncSession=Depends(get_db), _usage_check: bool=Depends(check_usage_limit("ai_messages"))):

    async def event_stream():
        yield _sse_event("phase", {"phase": "perception", "status": "started"})
        try:
            base_response = await _execute_ai_chat(payload=payload, user_id=user_id, db=db, _usage_check=_usage_check)
            ...
            yield _sse_event("phase", {"phase": "action", "status": "completed" if action_type and action_type != "none" else "idle"})
            yield _sse_event("phase", {"phase": "reflection", "status": "completed" if base_response.milestone else "idle"})
            yield _sse_event("done", {"result": base_response.result, "model_used": base_response.model_used, "action": base_response.action, "milestone": base_response.milestone})
        except Exception as exc:
