...
class TestCheckQuotaRedis:
    ...
    async def test_allowed_when_tokens_available(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            allowed, tokens_left, retry_after = await middleware._check_quota_redis(user_id="user-1", path="/api/v1/bookings", config=AI_QUOTA_CONFIG["/api/v1/bookings"])
    ...
    async def test_denied_when_tokens_exhausted(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            allowed, tokens_left, retry_after = await middleware._check_quota_redis(user_id="user-2", path="/api/v1/bookings", config=AI_QUOTA_CONFIG["/api/v1/bookings"])
    ...
    async def test_eval_called_with_correct_key_format(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            await middleware._check_quota_redis(user_id="user-abc", path="/api/v1/ai/chat", config=AI_QUOTA_CONFIG["/api/v1/ai/chat"])
    ...
    async def test_eval_receives_capacity_and_refill_rate(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            await middleware._check_quota_redis(user_id="user-x", path="/api/v1/ai/chat", config=config)
        call_args = mock_redis.eval.call_args
        positional_args = call_args[0]
    ...
    async def test_retry_after_calculated_from_cost_and_tokens(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            _, _, retry_after = await middleware._check_quota_redis(user_id="user-rate", path="/api/v1/bookings", config=config)
    ...
    async def test_retry_after_is_zero_when_allowed(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            allowed, _, retry_after = await middleware._check_quota_redis(user_id="user-ok", path="/api/v1/bookings", config=AI_QUOTA_CONFIG["/api/v1/bookings"])
        assert allowed is True
        assert retry_after == 0
    ...
    async def test_exception_falls_back_to_allow(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            allowed, tokens_left, retry_after = await middleware._check_quota_redis(user_id="user-err", path="/api/v1/bookings", config=AI_QUOTA_CONFIG["/api/v1/bookings"])
        assert allowed is True
    ...
    async def test_no_register_script_usage(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            await middleware._check_quota_redis(user_id="user-noscript", path="/api/v1/bookings", config=AI_QUOTA_CONFIG["/api/v1/bookings"])
    ...
    async def test_partial_tokens_left_returns_float(self, middleware, mock_redis):
        ...
        with patch("backend.utils.quota_middleware.get_redis", return_value=mock_redis):
            _, tokens_left, _ = await middleware._check_quota_redis(user_id="user-float", path="/api/v1/bookings", config=AI_QUOTA_CONFIG["/api/v1/bookings"])
...
class TestAIQuotaMiddlewareDispatch:
    ...
    async def test_raises_ai_quota_exceeded_when_denied(self, middleware):
        ...
        call_next = AsyncMock(return_value=MagicMock(headers={}))
        with patch.object(middleware, "_get_user_id", new=AsyncMock(return_value="user-denied")), patch.object(middleware, "_check_quota_redis", new=AsyncMock(return_value=(False, 0.0, 42))), pytest.raises(AIQuotaExceeded) as exc_info:
            await middleware.dispatch(request, call_next)
    ...
    async def test_quota_remaining_header_added_on_success(self, middleware):
        ...
        with patch.object(middleware, "_get_user_id", new=AsyncMock(return_value="user-ok")), patch.object(middleware, "_check_quota_redis", new=AsyncMock(return_value=(True, 7.0, 0))):
...
class TestQuotaModuleConstants:
    ...
    def test_quota_lua_script_constant_not_exported(self):
        ...
        assert not hasattr(mod, "QUOTA_LUA_SCRIPT"), "QUOTA_LUA_SCRIPT was removed from module scope in this PR; it should not be a module attribute"
