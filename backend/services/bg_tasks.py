...
async def enqueue_event_confirmation(
    email: str, event_title: str, event_time: str, meeting_link: Optional[str] = None
):
...
async def enqueue_event_update(
    email: str, event_title: str, event_time: str, meeting_link: Optional[str] = None
):
    ...
    await pool.enqueue_job(
        ...
        email,
        "update.html",
        "Update: Meeting Details Changed - GraftAI",
        {
            "event_title": event_title,
            "event_time": event_time,
            "meeting_link": meeting_link,
...
async def enqueue_event_cancellation(email: str, event_title: str, event_time: str):
    ...
    await pool.enqueue_job(
        ...
        email,
        "cancellation.html",
        "Cancelled: Meeting Removed - GraftAI",
        {"event_title": event_title, "event_time": event_time},
    )
...
async def enqueue_account_deletion(email: str):
    ...
    await pool.enqueue_job(
        ...
        email,
        "goodbye.html",
        "Account Successfully Deleted - GraftAI",
        {},
    )
...
async def enqueue_sync_error_alert(email: str, error_message: str):
    ...
    await pool.enqueue_job(
        ...
        email,
        "sync_error.html",
        "Action Required: Calendar Sync Failed - GraftAI",
...
async def enqueue_conflict_alert(email: str, requested_event: str, existing_event: str):
    ...
    await pool.enqueue_job(
        ...
        email,
        "conflict_alert.html",
        "Schedule Conflict Alert - GraftAI",
        {"requested_event": requested_event, "existing_event": existing_event},
    )
