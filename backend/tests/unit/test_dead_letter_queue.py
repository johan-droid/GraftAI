...
from backend.utils.dead_letter_queue import DeadLetterQueue, DLQItem, get_dlq
...
class TestDLQItemModel:
    ...
    def test_dlq_item_creation(self):
        ...
        item = DLQItem(id=str(uuid4()), action_type="send_email", payload={"to": "test@example.com", "subject": "Test"}, error_message="Timeout", status="pending", max_retries=3, retry_count=0, created_at=datetime.now(UTC), updated_at=datetime.now(UTC), next_retry_at=datetime.now(UTC) + timedelta(minutes=1), last_error_at=datetime.now(UTC), context={"user_id": "user-123"})
    ...
    def test_dlq_item_to_dict(self):
        ...
        item = DLQItem(id="item-123", action_type="send_email", payload={"to": "test@example.com"}, error_message="Error", status="pending", max_retries=3, retry_count=1, created_at=now, updated_at=now, next_retry_at=now, last_error_at=now, context={"user_id": "user-123"})
        data = item.to_dict()
...
class TestDeadLetterQueueEnqueue:
    ...
    async def test_enqueue_creates_item(self, mock_get_db, dlq):
        ...
        item_id = await dlq.enqueue(action_type="send_email", payload={"to": "test@example.com", "subject": "Hello"}, error="SendGrid timeout", max_retries=3, context={"user_id": "user-123"})
    ...
    async def test_enqueue_sets_correct_next_retry(self, mock_get_db, dlq):
        ...
        mock_session = _make_mock_db_session(mock_get_db)
        before_enqueue = datetime.now(UTC)
        await dlq.enqueue(action_type="send_sms", payload={"to": "+1234567890", "body": "Hello"}, error="Twilio error")
        after_enqueue = datetime.now(UTC)
...
class TestDeadLetterQueueDequeue:
    ...
    async def test_dequeue_pending_returns_ready_items(self, mock_get_db, dlq):
        ...
        mock_item.next_retry_at = datetime.now(UTC) - timedelta(minutes=1)
        mock_item.last_error_at = datetime.now(UTC)
        mock_item.context = {}
        mock_result = MagicMock()
        mock_result.scalars.return_value.all.return_value = [mock_item]
        _make_mock_db_session(mock_get_db, mock_result)
    ...
    async def test_dequeue_pending_skips_future_items(self, mock_get_db, dlq):
        ...
        mock_item = MagicMock()
        mock_item.next_retry_at = datetime.now(UTC) + timedelta(hours=1)
        mock_result = MagicMock()
...
class TestDeadLetterQueueRetryLogic:
    ...
    def test_retry_delays(self, dlq):
        ...
        expected_delays = [60, 300, 900, 3600, 7200]
        assert expected_delays == dlq.RETRY_DELAYS
...
class TestDeadLetterQueueProcessing:
    ...
    async def test_process_queue_with_handler(self, mock_get_db, dlq):
        ...
        mock_item.action_type = "send_email"
        mock_item.payload = {"to": "test@example.com"}
        dlq.dequeue_pending = AsyncMock(return_value=[DLQItem(id="item-1", action_type="send_email", payload={"to": "test@example.com"}, error_message=None, status="pending", max_retries=3, retry_count=0, created_at=datetime.now(UTC), updated_at=datetime.now(UTC), next_retry_at=None, last_error_at=None, context=None)])
    ...
    async def test_process_queue_no_handler(self, mock_get_db, dlq):
        ...
        dlq.dequeue_pending = AsyncMock(return_value=[DLQItem(id="item-1", action_type="send_email", payload={}, error_message=None, status="pending", max_retries=3, retry_count=0, created_at=datetime.now(UTC), updated_at=datetime.now(UTC), next_retry_at=None, last_error_at=None, context=None)])
...
class TestDeadLetterQueueStatistics:
    ...
    async def test_get_statistics(self, mock_get_db, dlq):
        ...
        mock_count_result.all.return_value = [("pending", 5), ("completed", 10), ("failed", 2)]
        mock_total_result = MagicMock()
        mock_total_result.scalar.return_value = 17
        mock_recent_result = MagicMock()
        mock_recent_result.scalar.return_value = 1
        mock_session = _make_mock_db_session(mock_get_db)
        mock_session.execute.side_effect = [mock_count_result, mock_total_result, mock_recent_result]
