...
tool_executions = Counter("tool_executions_total", "Total tool executions", ["tool_name", "status", "category"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
tool_duration = Histogram("tool_duration_seconds", "Tool execution duration", ["tool_name", "category"], buckets=[0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
tool_retry_count = Counter("tool_retry_count_total", "Number of tool retries", ["tool_name"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
agent_executions = Counter("agent_executions_total", "Total agent executions", ["agent_type", "status", "phase"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
agent_duration = Histogram("agent_duration_seconds", "Agent execution duration", ["agent_type", "phase"], buckets=[0.5, 1.0, 2.5, 5.0, 10.0, 30.0, 60.0], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
agent_phase_duration = Histogram("agent_phase_duration_seconds", "Duration of each agent phase", ["agent_type", "phase"], buckets=[0.1, 0.25, 0.5, 1.0, 2.5, 5.0], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
memory_operations = Counter("memory_operations_total", "Memory operations", ["layer", "operation"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
memory_latency = Histogram("memory_latency_seconds", "Memory operation latency", ["layer", "operation"], buckets=[0.001, 0.01, 0.025, 0.05, 0.1, 0.25, 0.5], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
memory_size = Gauge("memory_size_bytes", "Memory size in bytes", ["layer"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
llm_tokens_used = Counter("llm_tokens_used_total", "Total LLM tokens used", ["model", "operation"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
llm_api_calls = Counter("llm_api_calls_total", "Total LLM API calls", ["model", "status"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
llm_latency = Histogram("llm_latency_seconds", "LLM API call latency", ["model", "operation"], buckets=[0.5, 1.0, 2.5, 5.0, 10.0, 30.0], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
llm_cost = Counter("llm_cost_dollars", "Estimated LLM cost in dollars", ["model"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
active_automations = Gauge("active_automations", "Number of currently running automations", registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
automation_queue_size = Gauge("automation_queue_size", "Number of automations waiting in queue", registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
agent_errors = Counter("agent_errors_total", "Total agent errors", ["error_type", "component"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
booking_value_automated = Counter("booking_value_automated_dollars", "Total value of automated bookings", ["risk_level"], registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
attendee_satisfaction = Gauge("attendee_satisfaction_score", "Average attendee satisfaction (0-10)", registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
no_show_rate = Gauge("no_show_rate", "Percentage of no-shows after automation", registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
agent_build_info = Info("agent_build", "Agent build information", registry=CollectorRegistry() if PROMETHEUS_AVAILABLE else None)
