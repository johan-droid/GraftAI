...
class VideoConferenceConfig(Base):
    ...
    )  # Use as default for new bookings
    ...
    user: Mapped["UserTable"] = relationship(
        "UserTable", back_populates="video_conference_configs"
    )
    meetings: Mapped[List["VideoConferenceMeeting"]] = relationship(
        "VideoConferenceMeeting", back_populates="config", cascade="all, delete-orphan"
    )
...
class VideoConferenceMeeting(Base):
    ...
    config_id: Mapped[str] = mapped_column(
        String(100),
        ForeignKey("video_conference_configs.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    team_booking_id: Mapped[Optional[str]] = mapped_column(
        String(100),
        ForeignKey("team_bookings.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    provider_meeting_id: Mapped[str] = mapped_column(
    ...
    host_url: Mapped[Optional[str]] = mapped_column(
    ...
    settings: Mapped[dict] = mapped_column(
        JSON, default=dict
    )  # Snapshot of settings at creation
    ...
    recording_download_url: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True
    )
    ...
    # Relationships
    config: Mapped["VideoConferenceConfig"] = relationship(
        "VideoConferenceConfig", back_populates="meetings"
    )
...
class VideoConferenceRecording(Base):
    ...
    meeting_id: Mapped[str] = mapped_column(
        String(100),
        ForeignKey("video_conference_meetings.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    # Timestamps
    recording_started_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    recording_ended_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(timezone.utc)
