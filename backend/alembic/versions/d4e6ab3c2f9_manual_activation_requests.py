...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_manual_activation_requests_user_id",
        "manual_activation_requests",
        ["user_id"],
    )
    op.create_index(
        "ix_manual_activation_requests_status", "manual_activation_requests", ["status"]
    )
    op.create_index(
        "ix_manual_activation_requests_reviewed_by",
        "manual_activation_requests",
        ["reviewed_by"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_manual_activation_requests_status", table_name="manual_activation_requests"
    )
    op.drop_index(
        "ix_manual_activation_requests_user_id", table_name="manual_activation_requests"
    )
    op.drop_index(
        "ix_manual_activation_requests_reviewed_by",
        table_name="manual_activation_requests",
    )
