...
from backend.services.mail_service import render_template, send_email

logger = logging.getLogger(__name__)

async def send_push_notification(user_player_ids: list[str], heading: str, content: str, data: dict | None=None):
...
def _build_event_templates(notification_type: str, event_data: dict) -> tuple[str, str, str]:
    ...
    template_context = {"full_name": user_name, "event_title": title, "start_time": start_time, "end_time": end_time, "is_meeting": is_meeting or bool(meeting_link), "meeting_platform": platform_label, "meeting_link": meeting_link, "frontend_url": os.getenv("FRONTEND_BASE_URL", "https://graftai.tech").rstrip("/")}
...
async def _send_notification(recipient_emails: list[str], user_player_ids: list[str], content_type: str, event_data: dict):
    ...
    try:
        ...
        logger.info("Email notification queued for %s recipients", len(recipient_emails))
    except Exception as e:
        logger.warning("Email notification failed: %s", e)
    if user_player_ids:
        try:
            headings = {"created": "Event created", "updated": "Event updated", "deleted": "Event deleted"}
            await send_push_notification(user_player_ids, heading=headings.get(content_type, "Notification"), content=f"{event_data.get('title')} {content_type}", data={"event_id": str(event_data.get("id"))})
...
class NotificationService:
    ...
    async def send_booking_confirmation(self, to_email: str, booking_data: dict):
        ...
        message = f"Hello {attendee_name},\n\nYour booking '{title}' is confirmed for {start_time}{(' to ' + end_time if end_time else '')}.\nConfirmation code: {confirmation_code}\n"
        import html
        ...
        await send_custom_notification(user_email=to_email, subject=subject, message=message, html_body=f"<p>Hello {escaped_attendee},</p><p>Your booking <strong>{escaped_title}</strong> is confirmed for {escaped_start}{(' to ' + escaped_end if escaped_end else '')}.</p><p>Confirmation code: <strong>{escaped_code}</strong></p>", text_body=message)
    ...
    async def send_event_reminder(self, to_email: str, event_data: dict):
        ...
        await send_custom_notification(user_email=to_email, subject=subject, message=message, html_body=f"<p>Hello {user_name},</p><p>Reminder: <strong>{title}</strong> starts at {start_time}.</p>", text_body=message)
    ...
    async def send_team_invite(self, to_email: str, invite_data: dict):
        ...
        message = f"Hello,\n\n{inviter_name} invited you to join {team_name}.\n{('Accept invite: ' + invite_link if invite_link else '')}\n"
        import html
        ...
        await send_custom_notification(user_email=to_email, subject=subject, message=message, html_body=f"<p>Hello,</p><p>{escaped_inviter_name} invited you to join <strong>{escaped_team_name}</strong>.</p>{link_html}", text_body=message)

async def send_custom_notification(user_email: str, subject: str, message: str, html_body: str | None=None, text_body: str | None=None):
    if not html_body:
...
async def send_email_verification_code(user_email: str, full_name: str, code: str):
    ...
    template_context = {"full_name": full_name, "code": code, "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")}
    html_body = render_template("verification.html", template_context)
    text_body = f"Hello {full_name},\n\nYour GraftAI verification code is: {code}\n\nEnter this code on the verification page to complete your registration. This code expires in 15 minutes.\n\nIf you did not request this, please ignore this email.\n"
...
def _public_booking_links(booking_id: str, attendee_email: str) -> dict[str, str]:
    ...
    return {"token": token, "manage_url": f"{base}/public/bookings/{booking_id}?token={token}", "reschedule_url": f"{base}/public/bookings/{booking_id}/reschedule?token={token}", "cancel_url": f"{base}/public/bookings/{booking_id}/cancel?token={token}"}

async def send_booking_confirmation_to_attendee(attendee_email: str, attendee_name: str, organizer_name: str, event_title: str, attendee_start_time: str, attendee_end_time: str, attendee_zone: str, meeting_url: str | None=None, booking_id: str | None=None):
    links = _public_booking_links(booking_id, attendee_email) if booking_id else {"manage_url": "", "reschedule_url": "", "cancel_url": ""}
    subject = f"Booking confirmed: {event_title}"
    template_context = {"full_name": attendee_name, "organizer_name": organizer_name, "event_title": event_title, "start_time": attendee_start_time, "end_time": attendee_end_time, "attendee_zone": attendee_zone, "meeting_url": meeting_url, "manage_url": links.get("manage_url"), "reschedule_url": links.get("reschedule_url"), "cancel_url": links.get("cancel_url"), "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")}
    html_body = render_template("booking_confirmation_attendee.html", template_context)
    text_body = f"Hello {attendee_name},\n\nYour booking with {organizer_name} is confirmed.\nEvent: {event_title}\nTime: {attendee_start_time} - {attendee_end_time} ({attendee_zone})\nManage booking: {links.get('manage_url', '')}\nReschedule: {links.get('reschedule_url', '')}\nCancel: {links.get('cancel_url', '')}\n"
    await send_custom_notification(attendee_email, subject, text_body, html_body, text_body)

async def send_booking_confirmation_to_organizer(organizer_email: str, organizer_name: str, attendee_name: str, attendee_email: str, event_title: str, organizer_start_time: str, organizer_end_time: str, meeting_url: str | None=None, booking_id: str | None=None):
    links = _public_booking_links(booking_id, attendee_email) if booking_id else {"manage_url": ""}
    subject = f"New booking: {event_title}"
    template_context = {"full_name": organizer_name, "attendee_name": attendee_name, "attendee_email": attendee_email, "event_title": event_title, "start_time": organizer_start_time, "end_time": organizer_end_time, "meeting_url": meeting_url, "manage_url": links.get("manage_url"), "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")}
    html_body = render_template("booking_confirmation_organizer.html", template_context)
    text_body = f"Hello {organizer_name},\n\nA new booking has been created.\nAttendee: {attendee_name} <{attendee_email}>\nEvent: {event_title}\nTime: {organizer_start_time} - {organizer_end_time}\nManage booking: {links.get('manage_url', '')}\n"
    await send_custom_notification(organizer_email, subject, text_body, html_body, text_body)

async def send_booking_rescheduled_to_both(organizer_email: str, organizer_name: str, attendee_email: str, attendee_name: str, event_title: str, old_time: str, new_time: str, meeting_url: str | None=None, booking_id: str | None=None):
    links = _public_booking_links(booking_id, attendee_email) if booking_id else {"manage_url": "", "reschedule_url": "", "cancel_url": ""}
    subject = f"Booking rescheduled: {event_title}"
    organizer_context = {"full_name": organizer_name, "event_title": event_title, "old_time": old_time, "new_time": new_time, "meeting_url": meeting_url, "manage_url": links.get("manage_url"), "reschedule_url": links.get("reschedule_url"), "cancel_url": links.get("cancel_url"), "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")}
    attendee_context = {**organizer_context, "full_name": attendee_name}
    ...
    organizer_text = f"Hello {organizer_name},\n\nBooking '{event_title}' has been rescheduled.\nOld: {old_time}\nNew: {new_time}\nManage booking: {links.get('manage_url', '')}\n"
    attendee_text = f"Hello {attendee_name},\n\nBooking '{event_title}' has been rescheduled.\nOld: {old_time}\nNew: {new_time}\nManage booking: {links.get('manage_url', '')}\n"
...
async def send_booking_cancelled_to_both(organizer_email: str, organizer_name: str, attendee_email: str, attendee_name: str, event_title: str, original_time: str, cancelled_by: str, cancellation_reason: str | None=None, booking_id: str | None=None):
    links = _public_booking_links(booking_id, attendee_email) if booking_id else {"reschedule_url": ""}
    subject = f"Booking cancelled: {event_title}"
    organizer_context = {"full_name": organizer_name, "event_title": event_title, "original_time": original_time, "cancelled_by": cancelled_by, "cancellation_reason": cancellation_reason, "reschedule_url": links.get("reschedule_url"), "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")}
    attendee_context = {**organizer_context, "full_name": attendee_name}
    ...
    organizer_text = f"Hello {organizer_name},\n\nBooking '{event_title}' at {original_time} was cancelled by {cancelled_by}.\n" + (f"Reason: {cancellation_reason}.\n" if cancellation_reason else "")
    attendee_text = f"Hello {attendee_name},\n\nBooking '{event_title}' at {original_time} was cancelled by {cancelled_by}.\n" + (f"Reason: {cancellation_reason}.\n" if cancellation_reason else "")
    await send_custom_notification(organizer_email, subject, organizer_text, organizer_html, organizer_text)
    await send_custom_notification(attendee_email, subject, attendee_text, attendee_html, attendee_text)

async def send_booking_reminder_to_organizer(organizer_email: str, organizer_name: str, attendee_name: str, attendee_email: str, event_title: str, organizer_start_time: str, organizer_end_time: str, organizer_timezone: str, booking_id: str | None=None):
    base = os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")
    manage_url = f"{base}/dashboard/calendar"
    subject = f"Reminder: {event_title} starts in 24 hours"
    template_context = {"full_name": organizer_name, "attendee_name": attendee_name, "attendee_email": attendee_email, "event_title": event_title, "start_time": organizer_start_time, "end_time": organizer_end_time, "organizer_timezone": organizer_timezone, "booking_id": booking_id, "manage_url": manage_url, "frontend_url": base}
    html_body = render_template("booking_reminder_organizer.html", template_context)
    text_body = f"Hello {organizer_name},\n\nReminder: '{event_title}' is scheduled for {organizer_start_time} - {organizer_end_time} ({organizer_timezone}).\nAttendee: {attendee_name} <{attendee_email}>\nManage in dashboard: {manage_url}\n"
    await send_custom_notification(organizer_email, subject, text_body, html_body, text_body)

async def notify_quota_warning(user_id: str, user_email: str, full_name: str, feature: str, current_count: int, limit: int):
    feature_map = {"ai_messages": "AI Copilot Messages", "calendar_syncs": "Calendar Syncs"}
    ...
    template_context = {"full_name": full_name, "feature_label": feature_label, "current_count": current_count, "limit": limit, "usage_percent": usage_percent, "upgrade_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/") + "/pricing", "billing_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/") + "/dashboard/settings/billing", "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")}
    html_body = render_template("quota_warning.html", template_context)
    text_body = f"Hello {full_name},\n\nYou are using {current_count} of {limit} available {feature_label.lower()} today ({usage_percent}%). Upgrade to the Pro plan to remove this limit and keep GraftAI working without interruption. Visit {template_context['billing_url']} to upgrade now.\n\nThank you,\nThe GraftAI team"
    await send_custom_notification(user_email, subject, text_body, html_body, text_body)

async def notify_welcome_email(user_email: str, full_name: str):
    subject = "🚀 Welcome to GraftAI - Your AI Copilot is Ready!"
    template_context = {"full_name": full_name, "frontend_url": os.getenv("FRONTEND_BASE_URL", "http://localhost:3000")}
