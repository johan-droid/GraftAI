...
async def migrate_event_type_buffer_defaults(db: AsyncSession, batch_size: int=100) -> int:
    ...
    """
    stmt = select(EventTypeTable).where(or_(EventTypeTable.buffer_before_minutes.is_(None), EventTypeTable.buffer_after_minutes.is_(None))).limit(batch_size)
    ...
    if updated:
        await db.commit()
        logger.info("Migrated %s event_type record(s) to default buffer values.", updated)
    return updated

async def migrate(db: AsyncSession, batch_size: int=100) -> int:
