...
class DataRetentionManager:
    ...
    async def apply_retention_policies(self, db: AsyncSession) -> Dict[str, int]:
        ...
        stmt = select(DataRetentionSchedule).where(
            DataRetentionSchedule.auto_apply == True
        )
        ...
        for schedule in schedules:
            try:
                ...
                logger.error(
                    f"Failed to apply retention for {schedule.data_category}: {e}"
                )
    ...
    async def _apply_schedule(
        self, db: AsyncSession, schedule: DataRetentionSchedule
    ) -> int:
    ...
    async def _delete_expired_data(
        self, db: AsyncSession, category: str, cutoff: datetime
    ) -> int:
    ...
    async def _anonymize_expired_data(
        self, db: AsyncSession, category: str, cutoff: datetime
    ) -> int:
    ...
    async def _archive_expired_data(
        self, db: AsyncSession, category: str, cutoff: datetime
    ) -> int:
            ...
            report["schedules"].append(
                {
                    ...
                    "last_applied": schedule.last_applied_at.isoformat()
    ...
    async def get_data_inventory(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        event_count = (
            await db.execute(select(func.count()).select_from(EventTable))
        ).scalar() or 0
        booking_count = (
            await db.execute(select(func.count()).select_from(BookingTable))
        ).scalar() or 0
        token_count = (
            await db.execute(select(func.count()).select_from(UserTokenTable))
        ).scalar() or 0
        user_count = (
            await db.execute(select(func.count()).select_from(UserTable))
        ).scalar() or 0
        ...
        return {
            ...
            "data_inventory": {
                ...
                "user_tokens": {
                    "count": token_count,
                    "categories": ["authentication_data"],
                },
            },
        }
