...
class StorageService:
    ...
    def __init__(self):
        ...
        if all([self.bucket, self.access_key, self.secret_key]):
            self.client = boto3.client("s3", aws_access_key_id=self.access_key, aws_secret_access_key=self.secret_key, endpoint_url=self.endpoint, region_name=self.region)
    ...
    async def upload_file(self, file_obj: BinaryIO, remote_path: str, content_type: str) -> str | None:
        ...
        if not self.client:
            ...
            return remote_path
        try:
            self.client.upload_fileobj(file_obj, self.bucket, remote_path, ExtraArgs={"ContentType": content_type})
    ...
    def get_file_size(self, key: str) -> int:
        ...
        try:
            ...
            logger.warning("⚠️ Failed to get file size for %s: %s", key, e)
            return 0
    ...
    def delete_file(self, key: str) -> bool:
        ...
        try:
            ...
            logger.warning("⚠️ Failed to delete cloud artifact %s: %s", key, e)
            return False

    def get_presigned_upload_url(self, key: str, expires_in: int=3600, content_type: str | None=None) -> dict | None:
        ...
        try:
            ...
            url = self.client.generate_presigned_url("put_object", Params=params, ExpiresIn=expires_in)
