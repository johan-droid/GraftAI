...
class DLQService:
    ...
    async def get_pending_items(
        ...
        stmt = select(DeadLetterQueueItem).where(
            and_(
        ...
        ).order_by(DeadLetterQueueItem.created_at).limit(limit)
    ...
    async def mark_retrying(
        ...
        stmt = update(DeadLetterQueueItem).where(
    ...
    async def mark_failed(
        ...
        if new_retry_count >= item.max_retries:
            ...
            logger.error(f"💥 DLQ: Item {item_id[:8]}... permanently failed after {new_retry_count} retries")
            return False
        else:
            # Schedule next retry
            ...
            logger.info(f"🔄 DLQ: Item {item_id[:8]}... scheduled for retry {new_retry_count} (in {delay}s)")
            return True
    ...
    async def manual_resolve(
        ...
        logger.info(f"👤 DLQ: Item {item_id[:8]}... manually resolved by {user_id[:8]}...")
