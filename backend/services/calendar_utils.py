...
def generate_fingerprint(event_data: dict[str, Any]) -> str:
    ...
    return hashlib.sha256(f"{start}|{end}|{title}".encode()).hexdigest()

async def simple_upsert_event(db: AsyncSession, user_id: str, source: str, event_data: dict[str, Any]) -> EventTable:
    """Upsert a calendar event scanned from an external provider."""
    fingerprint = generate_fingerprint(event_data)
    external_id = event_data.get("external_id")
    stmt = select(EventTable).where(and_(EventTable.user_id == user_id, not EventTable.is_deleted, (EventTable.external_id == external_id) | (EventTable.fingerprint == fingerprint)))
    ...
    db_event_data = {k: v for k, v in event_data.items() if hasattr(EventTable, k) and k not in ["fingerprint", "source"]}
    db_event_data["title"] = _normalize_event_title(db_event_data.get("title"))
    new_event = EventTable(user_id=user_id, source=source, fingerprint=fingerprint, **db_event_data)
    db.add(new_event)
    return new_event
