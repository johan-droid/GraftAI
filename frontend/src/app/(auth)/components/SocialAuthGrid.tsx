...
interface SocialAuthGridProps {
  onSelect: (provider: "google" | "github" | "discord" | "microsoft") => void;
  loading?: boolean;
}

export default function SocialAuthGrid({ onSelect, loading }: SocialAuthGridProps) {
  const providers = [
    { 
      id: "google", 
      name: "Google",
    ...
    <div className="grid grid-cols-4 gap-3">
      {providers.map((provider) => (
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => onSelect(provider.id as "google" | "github" | "discord" | "microsoft")}
          disabled={loading}
          className="h-11 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center hover:bg-white/10 transition-all group overflow-hidden relative"
        >
          {provider.icon}
        </motion.button>
      ))}
