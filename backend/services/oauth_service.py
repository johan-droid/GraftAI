...
def sanitize_frontend_url(frontend_url: str | None) -> str:
    if not frontend_url:
...
def frontend_redirect_token(access_token: str, redirect_to: str="/dashboard", frontend_url: str | None=None, refresh_token: str | None=None) -> str:
...
def build_oauth_state(user_id: str | None, redirect_to: str | None="/dashboard", provider: str | None=None, frontend_url: str | None=None) -> str:
...
def parse_oauth_state(state: str) -> tuple[str | None, str, str | None, str | None]:
    if not state:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid OAuth state: missing state")
    parts = state.split(":", 6)
    if len(parts) not in (5, 6, 7):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid OAuth state: malformed")
    if len(parts) == 7:
    ...
    expected_signature = hmac.new(SECRET_KEY.encode(), payload.encode(), hashlib.sha256).hexdigest()[:16]
    if not hmac.compare_digest(signature, expected_signature):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid OAuth state: signature verification failed")
    try:
        state_time = int(timestamp_str)
        current_time = int(time.time())
        if current_time - state_time > OAUTH_STATE_EXPIRY_SECONDS:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid OAuth state: expired")
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid OAuth state: invalid timestamp")
