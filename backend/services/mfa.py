...
def _extract_device_fingerprints(preferences: dict) -> list[str]:
    stored = preferences.get("trusted_device_fingerprints") or preferences.get("device_fingerprints") or preferences.get("mfa_device_fingerprints")
    if stored is None:
        return []
...
async def _check_device_fingerprint_async(user_id: str, fingerprint: str) -> bool:
    ...
    try:
        ...
        logger.warning("Device fingerprint check unavailable for user %s: %s", user_id, exc)
        return False
...
async def verify_mfa_token(db: AsyncSession, user_id: str, token: str, temp_secret: str | None=None) -> bool:
