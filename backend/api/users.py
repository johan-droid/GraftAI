...
class UserProfileUpdateRequest(BaseModel):
    ...
    booking_layout: str | None = None
    default_calendar_id: str | None = None
    preferences: dict | None = None
    model_config = ConfigDict(extra="forbid")

    @field_validator("display_name", "full_name", "username", "bio", "phone", "timezone", "time_format", "theme", "brand_color_light", "brand_color_dark", "booking_layout", "default_calendar_id", mode="before")
    @classmethod
    ...
    def username_must_be_valid(cls, value: str | None):
        if value is None:
            return value
        if not re.fullmatch("[a-zA-Z0-9_-]{3,30}", value):
            msg = "Username must be 3-30 characters and contain only letters, numbers, hyphens, or underscores."
            raise ValueError(msg)
        return value.lower()

    @field_validator("display_name")
    def display_name_must_be_valid(cls, value: str | None):
        if value is None:
            return value
        normalized = value.strip()
        if len(normalized) < 1 or len(normalized) > 100:
            msg = "Display name must be 1-100 characters"
            raise ValueError(msg)
        if not re.fullmatch("[A-Za-z0-9 '\\-]+", normalized):
            msg = "Display name may only include letters, numbers, spaces, apostrophes, and hyphens"
            raise ValueError(msg)
        return normalized

    @field_validator("bio")
    def bio_length(cls, value: str | None):
        if value is None:
            return value
        if len(value.strip()) > 500:
            msg = "Bio must be under 500 characters"
            raise ValueError(msg)
        return value.strip()
    ...
    def validate_phone(cls, value: str | None):
        if value is None or not value.strip():
            return None
        normalized = value.strip()
        if not re.fullmatch("\\+?[1-9]\\d{1,14}", normalized):
    ...
    def validate_theme(cls, value: str | None):
        if value is None:
            return value
    ...
    def validate_booking_layout(cls, value: str | None):
        if value is None:
            return value
        if value not in {"monthly", "weekly", "daily"}:
            msg = "Invalid booking layout"
            raise ValueError(msg)
        return value
...
def _profile_setup_completed(user: UserTable) -> bool:
    ...
    if not isinstance(completed_steps, list):
        completed_steps = []
    try:
        completed_steps_set = {str(x) for x in completed_steps if x is not None}
    except Exception:
        completed_steps_set = set()
    return bool(prefs.get("profile_setup_completed") or user.onboarding_completed or "profile" in completed_steps_set)
...
def _apply_profile_payload(user: UserTable, payload: UserProfileUpdateRequest) -> UserTable:
    ...
    if payload.preferences is not None:
        filtered = {k: v for k, v in payload.preferences.items() if k not in {"display_name", "full_name", "username", "bio", "phone", "timezone", "time_format", "theme", "brand_color_light", "brand_color_dark", "booking_layout", "default_calendar_id", "role", "tier", "subscription_status", "is_superuser"}}
...
async def create_or_update_profile(payload: UserProfileUpdateRequest, current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    ...
    try:
        ...
        if payload.username is not None:
            username = payload.username.strip().lower()
            if username and username != user.username:
                stmt = select(UserTable).where(UserTable.username == username, UserTable.id != user.id)
...
async def get_profile_setup_status(current_user: UserTable=Depends(get_current_user)):
    ...
    return {"success": True, "message": "Setup status loaded successfully", "data": {"completed_steps": prefs.get("completed_steps", []), "profile_setup_completed": _profile_setup_completed(current_user), "profile_setup_skipped": _profile_setup_skipped(current_user), "onboarding_completed": bool(current_user.onboarding_completed), "profile": _serialize_profile(current_user)}}
...
async def get_google_calendar_auth_url(current_user: UserTable=Depends(get_current_user)):
    ...
    try:
        auth_url = await get_google_auth_url(state, prompt="consent")
    except ValueError as exc:
        logger.exception("Google OAuth configuration error for user %s: %s", current_user.id, exc)
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(exc))
    return {"success": True, "message": "Google authorization URL created", "data": {"auth_url": auth_url, "state": state}}

@router.post("/me/profile/skip-setup")
async def skip_profile_setup(current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    ...
    return {"success": True, "message": "Profile setup skipped successfully", "data": {"profile_setup_skipped": True, "profile_setup_completed": _profile_setup_completed(current_user), "onboarding_completed": bool(current_user.onboarding_completed)}}

@router.post("/me/profile/avatar")
async def upload_profile_avatar(file: UploadFile=File(...), current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    allowed_types = {"image/jpeg", "image/png", "image/webp"}
    if file.content_type not in allowed_types:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unsupported avatar image type. Use JPEG, PNG, or WEBP.")
    file.file.seek(0, 2)
    file_size = file.file.tell()
    if file_size > 5 * 1024 * 1024:
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Avatar file must be 5MB or smaller.")
    file.file.seek(0)
    header_sample = file.file.read(16)
    file.file.seek(0)
    magic_map = {"image/jpeg": b"\xff\xd8\xff", "image/png": b"\x89PNG", "image/webp": b"RIFF"}
    if file.content_type == "image/webp":
        if len(header_sample) < 12 or not (header_sample.startswith(b"RIFF") and header_sample[8:12] == b"WEBP"):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Avatar content does not match declared file type.")
...
async def complete_profile_setup_step(step_id: str, current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    ...
    return {"success": True, "message": "Onboarding step completed", "data": {"completed_steps": completed_steps, "next_step": next_step}}
...
async def get_onboarding_preview(current_user: UserTable=Depends(get_current_user)):
    ...
    return {"success": True, "message": "Onboarding preview loaded", "data": {"bookingPageUrl": f"/public/users/{username}", "isLive": bool(current_user.onboarding_completed)}}

@router.post("/me/onboarding/complete")
async def complete_onboarding_flow(current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    current_user.onboarding_completed = True
    current_user.onboarding_completed_at = datetime.now(UTC)
    await db.commit()
    return {"success": True, "message": "Onboarding completed successfully", "data": {"redirect_url": "/dashboard"}}

@router.get("/me/integrations")
async def get_my_integrations(current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    """Returns a list of connected services (e.g., ['google', 'microsoft'])."""
    active_stmt = select(UserTokenTable.provider).where(and_(UserTokenTable.user_id == current_user.id, UserTokenTable.is_active))
    inactive_stmt = select(UserTokenTable.provider).where(and_(UserTokenTable.user_id == current_user.id, not UserTokenTable.is_active))
    active_providers = (await db.execute(active_stmt)).scalars().all()
    inactive_providers = (await db.execute(inactive_stmt)).scalars().all()
    return {"active_providers": list(active_providers), "inactive_providers": sorted(set(inactive_providers) - set(active_providers))}

@router.delete("/me/integrations/{provider}")
async def disconnect_integration(provider: str, current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    """Deletes or deactivates the OAuth token for a specific provider."""
    stmt = select(UserTokenTable).where(and_(UserTokenTable.user_id == current_user.id, UserTokenTable.provider == provider))
...
@router.patch("/me")
async def update_current_user_profile(payload: UserProfileUpdateRequest, current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
...
async def create_current_user_out_of_office(payload: OutOfOfficeBlockCreateRequest, current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    blocks = _extract_out_of_office_blocks(current_user)
    block = {"id": uuid.uuid4().hex, "start_time": _to_utc_iso(payload.start_time), "end_time": _to_utc_iso(payload.end_time), "reason": (payload.reason or "").strip() or None, "created_at": datetime.now(UTC).isoformat()}
    blocks.append(block)
...
async def delete_current_user_out_of_office(block_id: str, current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    blocks = _extract_out_of_office_blocks(current_user)
...
@router.delete("/me")
async def delete_current_user_account(current_user: UserTable=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    ...
    await db.commit()
    return {"status": "account_anonymized", "message": "Your account has been anonymized and deactivated. Access has been removed."}
