...
def upgrade():
    ...
    op.add_column("integrations", sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True))
    op.add_column("integrations", sa.Column("is_deleted", sa.Boolean(), nullable=False, server_default=sa.false()))
    op.create_index("ix_integrations_is_deleted", "integrations", ["is_deleted"])
    op.add_column("webhook_subscriptions", sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True))
    op.add_column("webhook_subscriptions", sa.Column("is_deleted", sa.Boolean(), nullable=False, server_default=sa.false()))
    op.create_index("ix_webhook_subscriptions_is_deleted", "webhook_subscriptions", ["is_deleted"])
