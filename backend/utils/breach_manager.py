...
class BreachManager:
    ...
    async def report_breach(
        ...
        breach_ref = f"BRE-{datetime.utcnow().strftime('%Y%m%d')}-{secrets.token_hex(4).upper()}"
        ...
        logger.warning(f"Data breach reported: {breach_ref} - {breach.nature_of_breach}")
    ...
    async def notify_supervisory_authority(
        ...
        if breach.supervisory_notified:
            logger.warning(f"Supervisory authority already notified for breach {breach.breach_reference}")
            return breach
        ...
        if hours_since_discovery > self.SUPERVISORY_NOTIFICATION_DEADLINE:
            logger.error(f"Breach notification deadline exceeded: {hours_since_discovery:.1f} hours")
        ...
        logger.info(f"Supervisory authority notified for breach {breach.breach_reference}")
    ...
    async def notify_data_subjects(
        ...
        if breach.risk_level not in ["high", "critical"]:
            logger.info(f"Breach risk level {breach.risk_level} does not require data subject notification")
            return breach
        
        # Check if already notified
        if breach.data_subjects_notified:
            logger.warning(f"Data subjects already notified for breach {breach.breach_reference}")
            return breach
    ...
    async def get_breach_report(self, db: AsyncSession, breach_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "supervisory_notified_at": breach.supervisory_notified_at.isoformat() if breach.supervisory_notified_at else None,
            "supervisory_authority": breach.supervisory_authority,
            "data_subjects_notified": breach.data_subjects_notified,
            "data_subjects_notified_at": breach.data_subjects_notified_at.isoformat() if breach.data_subjects_notified_at else None,
    ...
    async def _notify_dpo(self, breach: DataBreachRecord):
        ...
        logger.critical(f"DPO ALERT: Breach {breach.breach_reference} - {breach.nature_of_breach}")
    ...
    async def _send_supervisory_notification(
        ...
        logger.info(f"Sending breach notification to {authority}: {notification['breach_reference']}")
    ...
    async def _send_subject_notifications(
        ...
        logger.info(f"Sending subject notifications for breach {breach.breach_reference}")
