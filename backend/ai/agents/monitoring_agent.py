...
class MonitoringAgent(BaseAgent):
    ...
    async def _generate_report(
        ...
        if report_type == "summary":
            report.update(
                await self._generate_summary_report(user_id, start_date, end_date)
            )

        elif report_type == "bookings":
            report.update(
                await self._generate_bookings_report(user_id, start_date, end_date)
            )
            ...
            report.update(
                await self._generate_engagement_report(user_id, start_date, end_date)
            )
    ...
    async def _check_for_issues(self, outcome_data: Dict[str, Any]) -> List[Dict]:
        ...
        if (
            outcome_data.get("details", {}).get("duration_ms", 0)
            > self.alert_thresholds["api_response_time_ms"]
        ):
            alert = await self._create_alert(
                level=AlertLevel.WARNING,
    ...
    async def _generate_bookings_report(
        ...
        return {
            "total_bookings": 12,
            "bookings_by_type": {"interview": 3, "review": 4, "general": 5},
            "bookings_by_day": {
                "Monday": 2,
                "Tuesday": 4,
                "Wednesday": 3,
