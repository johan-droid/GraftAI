...
class DSRRecord(Base):
    ...
    request_details = Column(
        JSON, default=dict
    )  # Specific data categories, date ranges, etc.
...
class DSRAuditLog(Base):
    ...
    id = Column(String(100), primary_key=True, default=generate_uuid)
    dsr_id = Column(
        String(100), ForeignKey("dsr_records.id"), nullable=False, index=True
    )
...
class DataRetentionSchedule(Base):
    ...
    )  # legal_obligation, contract, consent, legitimate_interest
    legal_basis_details = Column(Text, nullable=True)

    # Action after retention period
    action_after_retention = Column(
        String(50), default="delete"
...
class DataProcessingRecord(Base):
    ...
    legal_basis = Column(
        String(50), nullable=False
    )  # consent, contract, legal_obligation, etc.
    legal_basis_details = Column(Text, nullable=True)
