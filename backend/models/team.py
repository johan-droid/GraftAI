...
class Team(Base):
    ...
    max_booking_notice: Mapped[int] = mapped_column(
        Integer, default=168
    )  # hours (1 week)
    ...
    event_types: Mapped[List["TeamEventType"]] = relationship(
        "TeamEventType", back_populates="team", cascade="all, delete-orphan"
    )
    bookings: Mapped[List["TeamBooking"]] = relationship(
        "TeamBooking", back_populates="team", cascade="all, delete-orphan"
    )
    resources: Mapped[List["Resource"]] = relationship(
        "Resource", back_populates="team"
    )
    automation_rules: Mapped[List["AutomationRule"]] = relationship(
        "AutomationRule", back_populates="team", cascade="all, delete-orphan"
    )
...
class TeamMember(Base):
    ...
    round_robin_weight: Mapped[int] = mapped_column(
        Integer, default=1
    ...
    user: Mapped["UserTable"] = relationship(
        "UserTable", back_populates="team_memberships"
    )
...
class TeamEventType(Base):
    ...
    available_hours: Mapped[dict] = mapped_column(
        JSON, default=lambda: {"start": "09:00", "end": "17:00"}
    )
    ...
    # Assignment
    assigned_members: Mapped[list] = mapped_column(
        JSON, default=list
    )  # List of user IDs
    assignment_type: Mapped[str] = mapped_column(
        String(50), default="all"
    )  # all, specific, round_robin

    # Booking link
    booking_link_slug: Mapped[Optional[str]] = mapped_column(
    ...
    bookings: Mapped[List["TeamBooking"]] = relationship(
        "TeamBooking", back_populates="event_type", cascade="all, delete-orphan"
    )
...
class TeamBooking(Base):
    ...
    event_type_id: Mapped[str] = mapped_column(
        String,
        ForeignKey("team_event_types.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    event_type: Mapped["TeamEventType"] = relationship(
        "TeamEventType", back_populates="bookings"
    )
