...
from backend.services.integrations.webhook_service import WebhookService, WebhookPayload, WebhookEventType
...
async def create_integration(
    ...
    return IntegrationResponse(
        ...
        last_success_at=new_integration.last_success_at.isoformat() if new_integration.last_success_at else None,
        last_error_at=new_integration.last_error_at.isoformat() if new_integration.last_error_at else None
    )
...
async def list_integrations(
    ...
    return [
        IntegrationResponse(
            ...
            last_success_at=i.last_success_at.isoformat() if i.last_success_at else None,
...
async def test_integration(
    ...
    return {
        ...
        "status_code": result.get("status_code"),
        "message": "Test webhook sent successfully" if result["success"] else f"Failed: {result.get('error', 'Unknown error')}"
    }
...
async def get_integration_logs(
    ...
    ).order_by(desc(IntegrationLog.sent_at)).limit(limit)
