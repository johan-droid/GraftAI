...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('booking_id', sa.String(100), sa.ForeignKey('bookings.id', ondelete='CASCADE'), nullable=False, index=True),
    ...
    op.create_table(
        ...
        # Status
        sa.Column('status', sa.String(50), default='pending'),  # pending, retrying, failed, resolved
        
        # Resolution
        sa.Column('resolution', sa.String(50), nullable=True),  # manual, auto, ignored
        sa.Column('resolved_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('resolved_by', sa.String(100), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True),
