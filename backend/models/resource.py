...
class Resource(Base):
    ...
    max_booking_duration: Mapped[int] = mapped_column(
        Integer, default=480
    )  # minutes (8 hours)
    ...
    owner: Mapped["UserTable"] = relationship(
        "UserTable", back_populates="owned_resources"
    )
...
class ResourceBooking(Base):
    ...
    booking_id: Mapped[Optional[str]] = mapped_column(
        String(100), ForeignKey("bookings.id", ondelete="SET NULL"), nullable=True
    )
    team_booking_id: Mapped[Optional[str]] = mapped_column(
        String(100), ForeignKey("team_bookings.id", ondelete="SET NULL"), nullable=True
    )
    ...
    attendees: Mapped[int] = mapped_column(
        Integer, default=1
    )  # Number of people using resource
    ...
    user: Mapped["UserTable"] = relationship(
        "UserTable", back_populates="resource_bookings"
    )
...
class ResourceMaintenance(Base):
    ...
    )  # routine, repair, cleaning, inspection
