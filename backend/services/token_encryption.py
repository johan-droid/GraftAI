...
def _get_fernet() -> Fernet | None:
    secret = os.getenv("OAUTH_TOKEN_ENCRYPTION_KEY") or os.getenv("TOKEN_ENCRYPTION_KEY") or os.getenv("SECRET_KEY")
    if not secret:
        return None
    try:
        return Fernet(_derive_fernet_key(secret))
    except Exception as exc:
        logger.exception("Failed to initialize OAuth token encryption: %s", exc)
        return None
...
def decrypt_token_value(value: str | None) -> tuple[str | None, bool]:
    """
    ...
    try:
        ...
        logger.exception("Failed to decrypt OAuth token: invalid ciphertext")
    except Exception as exc:
        logger.exception("Failed to decrypt OAuth token: %s", exc)
    return (None, False)
