...
class AutomationRule(Base):
    ...
    confidence_threshold: Mapped[float] = mapped_column(
        Float, default=70.0
    )  # Min confidence to auto-execute
    ...
    user: Mapped["UserTable"] = relationship(
        "UserTable", back_populates="automation_rules"
    )
    team: Mapped[Optional["Team"]] = relationship(
        "Team", back_populates="automation_rules"
    )
...
class AutomationExecution(Base):
    ...
    )  # booking_created, conflict_detected, etc.
    ...
    automation_tier: Mapped[str] = mapped_column(
        String(20), default="draft"
    )  # draft, trusted, full_auto
    ...
    rule: Mapped["AutomationRule"] = relationship(
        "AutomationRule", back_populates="executions"
    )
...
class AutomationTemplate(Base):
    ...
    category: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # scheduling, conflict, team, resource
