...
class EmailParserService:
    ...
    TIME_PATTERNS = ["(\\d{1,2}):?(\\d{2})?\\s*(am|pm|AM|PM)", "(\\d{1,2}):?(\\d{2})?(am|pm)"]
    DATE_PATTERNS = ["\\b(tomorrow|today|tonight|next\\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday))\\b", "\\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\\b", "((?:january|february|march|april|may|june|july|august|september|october|november|december)\\s+\\d{1,2})", "(\\d{4}-\\d{2}-\\d{2})", "(\\d{1,2}/\\d{1,2}/\\d{2,4})"]
    DURATION_PATTERNS = ["(\\d+)\\s*(hour|hours|hr|hrs)", "(\\d+)\\s*(minute|minutes|min|mins)", "(\\d+)\\s*(half hour|half-hour)"]
    MEETING_KEYWORDS = ["meet", "meeting", "sync", "call", "discussion", "chat", "talk", "conference", "review", "standup", "stand-up", "1:1", "one-on-one", "interview", "presentation", "demo", "workshop", "brainstorm"]
    CONFIRMATION_KEYWORDS = ["confirmed", "accepted", "yes", "ill be there", "see you", "works for me", "sounds good", "im in", "count me in"]
    CANCELLATION_KEYWORDS = ["cancel", "cancelled", "cant make it", "wont be able", "need to reschedule", "something came up", "unavailable"]
    RESCHEDULE_KEYWORDS = ["reschedule", "move", "change", "different time", "another time", "wont work", "conflict", "overlap"]

    def __init__(self):
        self.timezone = "UTC"

    async def parse_email(self, subject: str, body: str, sender: str, received_at: datetime | None=None) -> ParsedMeeting:
        ...
        return ParsedMeeting(intent=intent, title=title, start_time=start_time, end_time=end_time, duration_minutes=duration, location=location, attendees=attendees, description=body[:500] if body else None, confidence=confidence, extracted_text=full_text[:1000], timezone=self.timezone)
    ...
    def _extract_title(self, subject: str, body: str) -> str | None:
        ...
        title = subject.strip()
        prefixes_to_remove = ["^re:\\s*", "^fw[d]?:\\s*", "^fwd:\\s*", "^meeting:\\s*", "^scheduled:\\s*"]
        for prefix in prefixes_to_remove:
            title = re.sub(prefix, "", title, flags=re.IGNORECASE)
        if len(title) < 10 or title.lower() in ["meeting", "sync", "call"]:
            body_patterns = ["regarding\\s*:?\\s*([^.\\n]+)", "re:\\s*([^.\\n]+)", "subject\\s*:?\\s*([^.\\n]+)"]
    ...
    def _extract_datetime(self, text: str, reference_time: datetime) -> datetime | None:
        ...
        date_mentioned = any(re.search(pattern, text, re.IGNORECASE) for pattern in self.DATE_PATTERNS)
        if date_mentioned:
            result = datetime.combine(base_date, datetime.min.time().replace(hour=9, minute=0))
            return result.replace(tzinfo=UTC)
        return None
        ...
        for pattern in self.DURATION_PATTERNS:
            ...
            if match:
                ...
                if "hour" in unit or unit in ["hr", "hrs"]:
                    return value * 60
                if "half hour" in unit:
                    return 30
                return value
        if "standup" in text_lower or "stand-up" in text_lower:
            return 15
        if "1:1" in text_lower or "one-on-one" in text_lower or "sync" in text_lower:
            return 30
    ...
    def _extract_location(self, text: str) -> str | None:
        ...
        if "zoom" in text_lower:
            return "Zoom"
        if "teams" in text_lower or "microsoft" in text_lower:
            return "Microsoft Teams"
        if "google meet" in text_lower or "meet" in text_lower:
            return "Google Meet"
        if "webex" in text_lower:
            return "Webex"
        location_patterns = ["(?:in|at|location|venue|place|room)\\s+(?:the\\s+)?([^.\\n,]+?(?:room|hall|center|building|office)[^.\\n,]*)"]
    ...
    def _calculate_confidence(self, intent: EmailIntent, title: str | None, start_time: datetime | None, location: str | None) -> float:
