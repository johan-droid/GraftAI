    ...
    while True:
        ...
        for key in keys:
            ...
            try:
                ...
                logger.exception("Unable to read queue length for %s: %s", key_name, exc)
                queue_length = -1
    ...
    while True:
        try:
            ...
            if depths:
                ...
                logger.info("Queue health - total backlog=%d, max queue depth=%d, queues=%s", total, max_depth, depths)
                if max_depth >= warning_threshold:
                    logger.warning("High queue backlog detected (threshold=%d): %s", warning_threshold, depths)
            else:
                logger.debug("Queue monitoring found no ARQ queue keys.")
            if failed_count:
                logger.warning("Detected %d failed queue job key(s).", failed_count)
        except Exception as exc:
            logger.exception("Queue monitoring encountered an error: %s", exc)
