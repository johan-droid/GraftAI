...
class BaseAgent:
    ...
    async def _phase_perception(self, context: AgentContext) -> Dict[str, Any]:
        ...
        understanding = await self._understand_state(
            trigger, enriched_context, memories
        )

        # Store understanding in short-term
        if context.memory.context:
            await context.memory.context.store_phase_result(
                phase="perception", key="understanding", value=understanding
            )
        ...
        perception = {
            ...
            "memory_layers_accessed": list(memories.keys())
    ...
    async def _phase_reflection(
        ...
        memory_updates = await self._update_memory_multi_layer(
            result, learnings, context
        )
        ...
        reflection = {
            ...
            "memory_layers_updated": ["short_term", "medium_term", "long_term"]
            if context.memory.manager
            else ["legacy"],
            "episodic_memory": {
        ...
        if context.memory.manager:
            await context.memory.manager.long_term.store_episode(
                reflection["episodic_memory"]
            )
    ...
    def _assess_outcome(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "evaluation": {
                ...
                "action_success": result.get("phases", {})
                .get("action", {})
                .get("status")
                == "completed",
