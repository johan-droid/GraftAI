...
class MultiLayerMemoryManager:
    ...
    async def _store_medium_term(
        self, key: str, value: Any, ttl_seconds: Optional[int]
    ):
    ...
    async def _retrieve_from_layer(self, key: str, layer: MemoryLayer) -> Optional[Any]:
        ...
        if isinstance(entry, dict) and "value" in entry:
            ...
            if expires:
                try:
                    ...
                    logger.warning(
                        f"Invalid expires_at in medium-term memory: {expires}"
                    )
    ...
    async def retrieve_by_context(self, context: str, n_results: int = 5) -> List[Dict]:
        ...
        long_term_results = await self.long_term.retrieve_similar_episodes(
            context, n_results
        )
...
class AgentMemoryContext:
    ...
    async def store_phase_result(
        self, phase: str, key: str, value: Any, promote_to_long_term: bool = False
    ):
    ...
    async def get_relevant_context(
        ...
        if include_short_term:
            ...
            context["short_term"] = [
                ...
                {
                    "type": "recent_tools",
                    "value": list(self.memory.short_term.tool_outputs.keys()),
                },
            ]
