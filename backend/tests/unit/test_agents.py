...
class TestBookingAgent:
    ...
    async def test_perception_phase_extracts_entities(self, booking_agent):
        ...
        context = {"user_message": "Schedule a meeting tomorrow at 2pm with john@example.com", "entities": {"date": "tomorrow", "time": "14:00", "attendees": ["john@example.com"]}}
    ...
    async def test_cognition_phase_evaluates_constraints(self, booking_agent):
        ...
        context = {"user_id": "test-user-123", "entities": {"date": "2024-01-15", "time": "14:00", "duration": 30}}
    ...
    async def test_action_phase_creates_booking(self, booking_agent):
        ...
        context = {"user_id": "test-user-123", "decision": {"action": "create_meeting", "title": "Test Meeting", "start_time": "2024-01-15T14:00:00", "duration": 30}}
    ...
    async def test_reflection_phase_evaluates_success(self, booking_agent):
        ...
        results = {"action": {"success": True, "booking_id": "booking-123"}}
    ...
    async def test_full_agent_execution_flow(self, booking_agent):
        ...
        context = {"user_id": "test-user-123", "user_message": "Book a 30min meeting tomorrow at 2pm", "entities": {"date": "tomorrow", "time": "14:00", "duration": 30}}
        with patch.object(booking_agent, "perception_phase", new_callable=AsyncMock) as mock_perceive, patch.object(booking_agent, "cognition_phase", new_callable=AsyncMock) as mock_cognize, patch.object(booking_agent, "action_phase", new_callable=AsyncMock) as mock_act, patch.object(booking_agent, "reflection_phase", new_callable=AsyncMock) as mock_reflect:
...
class TestAgentController:
    ...
    def controller(self, mock_llm_core, mock_vector_store, mock_graph_store):
        ...
        return AgentController(llm_core=mock_llm_core, vector_store=mock_vector_store, graph_store=mock_graph_store)
    ...
    async def test_dispatch_unknown_agent_type(self, controller):
        ...
        result = await controller.dispatch(agent_type=AgentType.BOOKING, user_id="test-user", context={})
    ...
    async def test_dispatch_successful_execution(self, controller):
        ...
        mock_agent.execute = AsyncMock(return_value={"success": True, "booking_id": "bk-123", "phases": {"perception": {"status": "completed"}, "cognition": {"status": "completed"}, "action": {"status": "completed"}, "reflection": {"status": "completed"}}})
        controller.register_agent(AgentType.BOOKING, mock_agent)
        result = await controller.dispatch(agent_type=AgentType.BOOKING, user_id="test-user", context={"message": "Book a meeting"})
...
class TestAgentResponse:
    ...
    def test_agent_response_creation(self):
        ...
        response = AgentResponse(request_id="req-123", agent_type=AgentType.BOOKING, success=True, result={"booking_id": "bk-123"}, processing_time_ms=150.5)
    ...
    def test_agent_response_with_error(self):
        ...
        response = AgentResponse(request_id="req-456", agent_type=AgentType.OPTIMIZATION, success=False, result={}, error="Failed to optimize schedule", processing_time_ms=500.0)
...
class TestAgentRequest:
    ...
    def test_agent_request_creation(self):
        ...
        request = AgentRequest(id="req-123", type=AgentType.BOOKING, user_id="user-456", context={"message": "Book a meeting"}, priority=3)
    ...
    def test_agent_request_default_priority(self):
        ...
        request = AgentRequest(id="req-123", type=AgentType.EXECUTION, user_id="user-456", context={})
