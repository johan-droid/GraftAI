...
class PublicBookingActionResponse(BaseModel):
    ...
    status: str
    message: str
    organizer_start_time: str | None = None
    organizer_end_time: str | None = None
    invitee_start_time: str | None = None
    invitee_end_time: str | None = None
...
async def _resolve_public_event_or_404(db: AsyncSession, username: str, event_type: str) -> tuple[UserTable, EventTypeTable]:
...
def _format_booking_times(booking: BookingTable, organizer_timezone: str) -> dict[str, str]:
    ...
    return {"organizer_start_time": booking.start_time.astimezone(organizer_tz).strftime("%Y-%m-%d %I:%M %p"), "organizer_end_time": booking.end_time.astimezone(organizer_tz).strftime("%Y-%m-%d %I:%M %p"), "invitee_start_time": booking.start_time.astimezone(invitee_tz).strftime("%Y-%m-%d %I:%M %p"), "invitee_end_time": booking.end_time.astimezone(invitee_tz).strftime("%Y-%m-%d %I:%M %p"), "invitee_zone": str(invitee_tz)}

def _frontend_base_url() -> str:
    return os.getenv("FRONTEND_BASE_URL", "http://localhost:3000").rstrip("/")

def _build_public_action_links(booking_id: str, attendee_email: str, token: str | None=None) -> dict[str, str]:
    action_token = token or create_public_action_token(booking_id, attendee_email)
    base = _frontend_base_url()
    return {"action_token": action_token, "reschedule_url": f"{base}/public/bookings/{booking_id}/reschedule?token={action_token}", "cancel_url": f"{base}/public/bookings/{booking_id}/cancel?token={action_token}", "manage_url": f"{base}/public/bookings/{booking_id}?token={action_token}"}
...
async def get_public_stats(db: AsyncSession=Depends(get_db)):
    ...
    deleted_count_stmt = select(func.count(AuditLogTable.id)).where(and_(AuditLogTable.event_type == "user_delete", AuditLogTable.result == "success"))
    deleted_accounts = (await db.execute(deleted_count_stmt)).scalar() or 0
    return {"registered_users": registered_users, "live_visitors": max(1, live_visitors), "deleted_accounts": deleted_accounts}

@router.post("/public/heartbeat")
async def public_heartbeat(payload: HeartbeatRequest):
    """Register a visitor as 'Live'."""
    _LIVE_SESSIONS[payload.session_id] = datetime.now(UTC)
    return {"status": "ok"}

@router.get("/public/events/{username}/{event_type}", response_model=PublicEventResponse)
async def get_public_event_details(username: str, event_type: str, db: AsyncSession=Depends(get_db)):
    user, event_type_obj = await _resolve_public_event_or_404(db, username, event_type)
    return {"title": event_type_obj.name, "description": event_type_obj.description, "duration_minutes": event_type_obj.duration_minutes, "meeting_provider": event_type_obj.meeting_provider, "username": user.username, "event_type_slug": event_type_obj.slug, "timezone": user.timezone or "UTC", "recurrence_rule": event_type_obj.recurrence_rule, "custom_questions": event_type_obj.custom_questions, "requires_attendee_confirmation": event_type_obj.requires_attendee_confirmation, "requires_payment": event_type_obj.requires_payment, "payment_amount": event_type_obj.payment_amount, "payment_currency": event_type_obj.payment_currency, "travel_time_before_minutes": event_type_obj.travel_time_before_minutes, "travel_time_after_minutes": event_type_obj.travel_time_after_minutes}
...
async def get_public_user_profile(username: str, db: AsyncSession=Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    return {"username": user.username, "full_name": user.full_name, "timezone": user.timezone or "UTC"}

@router.get("/public/users/{username}/{event_type}", response_model=PublicEventResponse)
async def get_public_user_event_details(username: str, event_type: str, db: AsyncSession=Depends(get_db)):
...
async def get_public_event_availability(request: Request, username: str, event_type: str, month: str=Query(..., description="Month in YYYY-MM format."), time_zone: str | None=Query(None, description="Optional guest timezone."), db: AsyncSession=Depends(get_db)):
...
@router.get("/public/users/{username}/{event_type}/availability", response_model=PublicAvailabilityResponse)
async def get_public_user_event_availability(request: Request, username: str, event_type: str, month: str=Query(..., description="Month in YYYY-MM format."), time_zone: str | None=Query(None, description="Optional guest timezone."), db: AsyncSession=Depends(get_db)):
    return await get_public_event_availability(request=request, username=username, event_type=event_type, month=month, time_zone=time_zone, db=db)

@router.get("/public/users/{username}/{event_type}/daily_availability", response_model=PublicDailyAvailabilityResponse)
async def get_public_user_event_availability_for_day(request: Request, username: str, event_type: str, date: str=Query(..., description="Date in YYYY-MM-DD format."), time_zone: str | None=Query(None, description="Optional guest timezone."), db: AsyncSession=Depends(get_db)):
    client_id = request.client.host if request.client else "anonymous"
    await rate_limit(client_id, api_limits["availability"])
    user, event_type_obj = await _resolve_public_event_or_404(db, username, event_type)
    try:
...
async def book_public_event(request: Request, username: str, event_type: str, payload: PublicBookingRequest, db: AsyncSession=Depends(get_db)):
    if not validate_csrf_origin(request):
        logger.warning("🚫 CSRF rejected: Invalid origin from %s", request.client.host if request.client else "unknown")
        raise HTTPException(status_code=403, detail="Invalid request origin. Please use the official booking page.")
    client_id = request.client.host if request.client else "anonymous"
    ...
    try:
        await send_booking_confirmation_to_attendee(attendee_email=booking.email, attendee_name=booking.full_name, organizer_name=organizer_name, event_title=event_type_obj.name, attendee_start_time=formatted_times["invitee_start_time"], attendee_end_time=formatted_times["invitee_end_time"], attendee_zone=formatted_times["invitee_zone"], meeting_url=meeting_url, booking_id=booking.id)
        await send_booking_confirmation_to_organizer(organizer_email=user.email, organizer_name=organizer_name, attendee_name=booking.full_name, attendee_email=booking.email, event_title=event_type_obj.name, organizer_start_time=formatted_times["organizer_start_time"], organizer_end_time=formatted_times["organizer_end_time"], meeting_url=meeting_url, booking_id=booking.id)
    except Exception as exc:
        logger.warning("Booking emails failed for booking=%s: %s", booking.id, exc)
    return {"success": True, "booking_id": booking.id, "event_id": booking.event_id, "organizer_start_time": formatted_times["organizer_start_time"], "organizer_end_time": formatted_times["organizer_end_time"], "invitee_start_time": formatted_times["invitee_start_time"], "invitee_end_time": formatted_times["invitee_end_time"], "invitee_zone": formatted_times["invitee_zone"], "meeting_url": meeting_url, "action_token": links["action_token"], "manage_url": links["manage_url"], "reschedule_url": links["reschedule_url"], "cancel_url": links["cancel_url"]}
...
async def book_public_user_event(request: Request, username: str, event_type: str, payload: PublicBookingRequest, db: AsyncSession=Depends(get_db)):
    return await book_public_event(request=request, username=username, event_type=event_type, payload=payload, db=db)

@router.get("/public/bookings/{booking_id}", response_model=PublicBookingDetailsResponse)
async def get_public_booking_details(booking_id: str, token: str=Query(..., description="Signed action token from booking email"), db: AsyncSession=Depends(get_db)):
    ...
    meeting_url = booking_event.meeting_url if booking_event else None
    duration_minutes = event_type_obj.duration_minutes if event_type_obj and event_type_obj.duration_minutes else max(1, int((booking.end_time - booking.start_time).total_seconds() / 60))
    ...
    return {"booking_id": booking.id, "status": booking.status, "full_name": booking.full_name, "email": booking.email, "event_title": event_title, "organizer_name": organizer.full_name or organizer.username or organizer.email, "organizer_username": organizer.username or organizer.email, "organizer_timezone": organizer.timezone or "UTC", "event_type_slug": event_type_slug, "duration_minutes": duration_minutes, "organizer_start_time": formatted_times["organizer_start_time"], "organizer_end_time": formatted_times["organizer_end_time"], "invitee_start_time": formatted_times["invitee_start_time"], "invitee_end_time": formatted_times["invitee_end_time"], "invitee_zone": formatted_times["invitee_zone"], "meeting_url": meeting_url, "action_token": links["action_token"], "reschedule_url": links["reschedule_url"], "cancel_url": links["cancel_url"]}

@router.patch("/public/bookings/{booking_id}/reschedule", response_model=PublicBookingActionResponse)
async def reschedule_public_booking(request: Request, booking_id: str, payload: PublicRescheduleRequest, token: str=Query(..., description="Signed action token from booking email"), db: AsyncSession=Depends(get_db)):
    client_id = request.client.host if request.client else "anonymous"
    await rate_limit(client_id, api_limits["public_booking"])
    booking = await db.get(BookingTable, booking_id)
    if not booking:
    ...
    event_type_obj = await db.get(EventTypeTable, booking.event_type_id) if booking.event_type_id else None
    duration_minutes = event_type_obj.duration_minutes if event_type_obj else int((booking.end_time - booking.start_time).total_seconds() / 60)
    ...
    booking_conflict_stmt = select(BookingTable).where(and_(BookingTable.user_id == booking.user_id, BookingTable.id != booking.id, BookingTable.status.in_(["pending", "confirmed", "accepted", "rescheduled"]), BookingTable.start_time < new_end_utc, BookingTable.end_time > new_start_utc))
    ...
    event_conflict_stmt = select(EventTable).where(and_(EventTable.user_id == booking.user_id, EventTable.id != booking.event_id, EventTable.start_time < new_end_utc, EventTable.end_time > new_start_utc))
    ...
    _build_public_action_links(booking.id, booking.email, token=token)
    meeting_url = None
    ...
    try:
        await send_booking_rescheduled_to_both(organizer_email=organizer.email, organizer_name=organizer.full_name or organizer.username or organizer.email, attendee_email=booking.email, attendee_name=booking.full_name, event_title=event_title, old_time=old_time_label, new_time=new_time_label, meeting_url=meeting_url, booking_id=booking.id)
    except Exception as exc:
        logger.warning("Reschedule emails failed for booking=%s: %s", booking.id, exc)
    return {"success": True, "booking_id": booking.id, "status": booking.status, "message": "Booking rescheduled successfully", "organizer_start_time": formatted_times["organizer_start_time"], "organizer_end_time": formatted_times["organizer_end_time"], "invitee_start_time": formatted_times["invitee_start_time"], "invitee_end_time": formatted_times["invitee_end_time"], "invitee_zone": formatted_times["invitee_zone"]}

@router.delete("/public/bookings/{booking_id}", response_model=PublicBookingActionResponse)
async def cancel_public_booking(request: Request, booking_id: str, token: str=Query(..., description="Signed action token from booking email"), payload: PublicCancelRequest | None=Body(default=None), db: AsyncSession=Depends(get_db)):
    client_id = request.client.host if request.client else "anonymous"
    await rate_limit(client_id, api_limits["public_booking"])
    ...
    if not verify_public_action_token(booking.id, booking.email, token):
        raise HTTPException(status_code=403, detail="Invalid booking action token")
    if booking.status == "cancelled":
        return {"success": True, "booking_id": booking.id, "status": booking.status, "message": "Booking already cancelled"}
    ...
    if organizer:
        try:
            await send_booking_cancelled_to_both(organizer_email=organizer.email, organizer_name=organizer_name, attendee_email=booking.email, attendee_name=booking.full_name, event_title=event_title, original_time=original_time_label, cancelled_by="attendee", cancellation_reason=cancellation_reason, booking_id=booking.id)
        except Exception as exc:
            logger.warning("Cancellation emails failed for booking=%s: %s", booking.id, exc)
    return {"success": True, "booking_id": booking.id, "status": booking.status, "message": "Booking cancelled successfully"}
