...
class SmartAction(BaseModel):
    ...
    action_type: str = Field(
        ..., description="e.g., 'schedule', 'cancel', 'reschedule'"
    )
    title: str
    description: str
...
async def suggest_action(
    ...
    if "dashboard" in context:
        suggestion = (
            "Review your next meetings and ask me to reschedule if anything conflicts."
        )
        smart_actions = [
            SmartAction(
                id="action_dash_1",
        ...
        suggestion = (
            "Try asking for your next availability or to summarize today’s schedule."
        )
        smart_actions = [
            SmartAction(
                id="action_gen_1",
