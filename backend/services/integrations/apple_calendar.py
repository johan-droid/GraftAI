        ...
        try:
            ...
            for cal in calendars:
                result.append({
                    ...
                    "name": cal.name,
                    "url": str(cal.url),
                    "supported_components": cal.get_supported_components() if hasattr(cal, 'get_supported_components') else ["VEVENT"],
                })
    ...
    async def get_events(
        ...
        try:
            ...
            for event in events:
                ...
                event_data = {
                    "id": event.id,
                    "icalendar_uid": str(vevent.uid.value) if hasattr(vevent, 'uid') else None,
                    ...
                    "created": vevent.created.value if hasattr(vevent, 'created') else None,
                    "last_modified": vevent.last_modified.value if hasattr(vevent, 'last_modified') else None,
                    "organizer": str(vevent.organizer.value) if hasattr(vevent, 'organizer') else None,
                    "attendees": [str(att.value) for att in vevent.attendee_list] if hasattr(vevent, 'attendee_list') else [],
                    "status": str(vevent.status.value) if hasattr(vevent, 'status') else "confirmed",
                    "recurrence_id": str(vevent.recurrence_id.value) if hasattr(vevent, 'recurrence_id') else None,
