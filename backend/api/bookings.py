...
from backend.models.tables import UserTable, BookingTable, AIAutomationTable, generate_uuid
...
def _track_automation_start(booking_id: str, task: asyncio.Task, automation_id: Optional[str] = None) -> str:
    """Track the start of an automation task"""
    automation_id = automation_id or f"auto_{booking_id}_{datetime.utcnow().timestamp()}"
...
async def create_booking(
    ...
    try:
        ...
        attendee_email = booking_data.attendees[0] if booking_data.attendees else current_user.email
        attendee_name = attendee_email.split('@')[0]  # Use email prefix as name fallback
        ...
        automation_id = _track_automation_start(booking.id, task, automation_id=automation_id)
        ...
        if idempotency_key:
            await store_idempotency_key(
                ...
                booking_data.model_dump(), response_data, 201
            )
    ...
    try:
        ...
        if automation_record:
            ...
            return AutomationStatusResponse(
                ...
                automation_id=automation_record.id,
                started_at=automation_record.started_at.isoformat() if automation_record.started_at else None,
                completed_at=automation_record.completed_at.isoformat() if automation_record.completed_at else None,
                decision_score=automation_record.decision_score,
                risk_assessment=automation_record.risk_assessment,
                actions_completed=sum(1 for action in actions if isinstance(action, dict) and action.get("success")),
    ...
    try:
        ...
        if automation_record and automation_record.status in {"completed", "partial"}:
            ...
            return AutomationResultResponse(
                ...
                timestamp=(automation_record.completed_at or automation_record.created_at).isoformat(),
        ...
        while attempts < max_attempts:
            ...
            if automation_record:
                current_status = {
                    ...
                    "timestamp": (automation_record.completed_at or automation_record.created_at).isoformat() if automation_record.completed_at or automation_record.created_at else None,
