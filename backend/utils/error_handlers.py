...
def _get_request_meta(request: Request) -> dict[str, Any]:
    ...
    return {"path": request.url.path, "method": request.method, "user_id": user_id}
...
async def http_exception_handler(request: Request, exc: Exception):
    ...
    logger.warning("API HTTPException", extra={"error": str(exc.detail), "status_code": exc.status_code, "request_id": request_id, **meta})
    ...
    content = {"error": user_message, "code": "http", "detail": exc.detail}
    if request_id:
...
async def validation_exception_handler(request: Request, exc: Exception):
    ...
    meta = _get_request_meta(request)
    request_id = getattr(request.state, "request_id", None)
    logger.warning("API validation error", extra={"error": str(exc), "status_code": 422, "request_id": request_id, **meta})
    content = {"error": "Invalid request payload.", "code": "validation"}
    if request_id:
        content["request_id"] = request_id
    if not _is_production():
        normalized_errors = []
        for err in exc.errors():
            normalized_errors.append({"loc": list(err.get("loc", [])), "msg": err.get("msg"), "type": err.get("type")})
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    meta = _get_request_meta(request)
    request_id = getattr(request.state, "request_id", None)
    logger.error("API error", extra={"error": str(exc), "category": category, "path": request.url.path, "method": request.method, "status_code": status_code, "request_id": request_id, **meta}, exc_info=True)
    payload: dict[str, Any] = {"error": _get_user_message(category, exc), "code": category}
    if request_id:
