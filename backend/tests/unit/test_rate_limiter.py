...
class MockRequest:
    ...
    def __init__(self, path="/api/v1/test", client_host="127.0.0.1", headers=None, user=None):
        self.url = MagicMock()
        self.url.path = path
...
class TestRateLimitStrategies:
    ...
    async def memory_limiter(self):
        ...
        return RateLimiter(redis_client=None, default_limit=10, default_window=60, strategy=RateLimitStrategy.SLIDING_WINDOW)
    ...
    async def test_sliding_window_in_memory(self, memory_limiter):
        ...
        request = MockRequest(path="/api/v1/test")
        for i in range(10):
            allowed, _remaining, _retry_after = await memory_limiter.is_allowed(request)
    ...
    async def test_rate_limit_key_generation(self, memory_limiter):
        ...
        request = MockRequest(path="/api/v1/test", client_host="192.168.1.1")
...
class TestRateLimitMiddleware:
    ...
    async def test_middleware_skips_health_checks(self):
        ...
        app = MagicMock()
        middleware = RateLimitMiddleware(app=app, skip_paths=["/health", "/docs"])
...
class TestRedisIntegration:
    ...
    async def test_redis_connection_attempt(self, mock_from_url):
        ...
        get_rate_limiter(redis_url="redis://localhost:6379")
...
class TestRateLimitEdgeCases:
    ...
    async def test_rate_limit_with_forwarded_header(self):
        ...
        request = MockRequest(headers={"X-Forwarded-For": "10.0.0.1, 10.0.0.2, 10.0.0.3"})
    ...
    async def test_rate_limit_reset_after_window(self):
        ...
        limiter = RateLimiter(redis_client=None, default_limit=2, default_window=1)
        limiter.strategy = RateLimitStrategy.FIXED_WINDOW
        request = MockRequest(path="/test")
        for _ in range(2):
            allowed, _, _ = await limiter.is_allowed(request)
            assert allowed is True
        allowed, _, _retry_after = await limiter.is_allowed(request)
        assert allowed is False
    ...
    @pytest.mark.asyncio
    async def test_tiered_client_identifier_with_api_key(self):
        """Test that client identifier includes tier when api_key_tiers mapping provided."""
        limiter = RateLimiter(redis_client=None)
        limiter.api_key_tiers = {"test-api-key-123": "pro"}
        request = MockRequest(headers={"X-API-Key": "test-api-key-123"})
        client_id = limiter._get_client_identifier(request)
        assert client_id.startswith("tier:pro:apikey:test-api-key-123")

    @pytest.mark.asyncio
    async def test_streaming_endpoint_limits_are_tiered(self):
        """Streaming endpoints should get stricter, tiered limits."""
        limiter = RateLimiter(redis_client=None)
        free_limit = limiter._get_endpoint_limit("/api/v1/ai/stream", tier="free")
        pro_limit = limiter._get_endpoint_limit("/api/v1/ai/stream", tier="pro")
        ent_limit = limiter._get_endpoint_limit("/api/v1/ai/stream", tier="enterprise")
        assert free_limit == (5, 60)
        assert pro_limit == (20, 60)
        assert ent_limit == (200, 60)
...
class TestRateLimitStrategiesComparison:
    ...
    async def test_sliding_window_vs_fixed_window(self):
        ...
        fixed_limiter = RateLimiter(redis_client=None, default_limit=10, default_window=60, strategy=RateLimitStrategy.FIXED_WINDOW)
        sliding_limiter = RateLimiter(redis_client=None, default_limit=10, default_window=60, strategy=RateLimitStrategy.SLIDING_WINDOW)
        request = MockRequest(path="/test")
        for _ in range(5):
...
class TestRateLimiterTypingImport:
    ...
    def test_any_not_in_module_imports(self):
        ...
        import backend.utils.rate_limiter as mod
        assert not hasattr(mod, "Any"), "Any was removed from rate_limiter imports in this PR; it should not be a module-level name"
