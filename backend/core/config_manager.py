...
class PositiveIntegerValidator(ConfigValidator):
    ...
    def __init__(self, min_value: int = 1, max_value: Optional[int] = None):
...
class ConfigManager:
    ...
    def _setup_default_schemas(self):
        ...
        schemas = [
            ...
            ConfigSchema(
                ...
                required=False,
                default=20,
                validator=PositiveIntegerValidator(1, 100),
            ...
            ConfigSchema(
                ...
                required=False,
                default=10,
                validator=PositiveIntegerValidator(1, 50),
            ...
            ConfigSchema(
                ...
                sensitive=True,
                source=ConfigSource.SECRETS_MANAGER
            ),
            ...
            ConfigSchema(
                ...
                type=bool,
                required=False,
                default=self.env == Environment.DEVELOPMENT,
            ...
            ConfigSchema(
                ...
                required=False,
                default=100,
                validator=PositiveIntegerValidator(1, 1000),
            ...
            ConfigSchema(
                ...
                required=False,
                default=30,
                validator=PositiveIntegerValidator(1, 300),
            ...
            ConfigSchema(
                ...
                required=False,
                default=9090,
                validator=PositiveIntegerValidator(1024, 65535),
            ...
            ConfigSchema(
                ...
                required=False,
                default=587,
                validator=PositiveIntegerValidator(1, 65535),
    ...
    def _load_config_sources(self):
        ...
        # Remote configuration (placeholder)
    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        if not schema:
            # If no schema, try to get from environment
            value = os.getenv(key, default)
            self.config_cache[key] = value
            return value
        
        # Load value from sources
        ...
        if value is not None and schema.validator:
            if not schema.validator.validate(value):
                ...
                logger.error(f"Configuration validation failed for {key}: {error_msg}")
                if schema.required:
                    raise ValueError(f"Required configuration {key} is invalid: {error_msg}")
                value = schema.default
    ...
    def _load_from_source(self, source: ConfigSource, key: str) -> Any:
        ...
        elif source == ConfigSource.SECRETS_MANAGER:
    ...
    def _load_from_file(self, key: str) -> Any:
        ...
        config_files = [
            f".env.{self.env.value}",
            f".env.{self.env.value}.local",
            "config.yaml",
            "config.json",
            f"config.{self.env.value}.yaml",
            f"config.{self.env.value}.json"
        ]
    ...
    def _load_from_remote(self, key: str) -> Any:
        ...
        # This would integrate with Consul, etcd, etc.
        # For now, return None (not implemented)
        return None
    ...
    def set(self, key: str, value: Any, persist: bool = False):
        ...
        logger.info(f"Configuration updated: {key} = {'[REDACTED]' if schema and schema.sensitive else value}")
    
    def _persist_value(self, key: str, value: Any, schema: Optional[ConfigSchema]):
        ...
        # Add other persistence logic as needed
    ...
    def reload_config(self, keys: Optional[List[str]] = None):
        ...
        logger.info(f"Reloaded configuration for {len(keys)} keys")
    ...
    async def watch_for_changes(self):
        ...
        # This would implement file watching, database polling, etc.
...
def initialize_config():
    ...
    if not validation_result.is_valid:
        ...
        for invalid in validation_result.invalid_types:
            logger.error(f"  - Invalid type: {invalid}")
    
    logger.info(f"Configuration initialized for {env.value} environment")
