...
def upgrade() -> None:
    ...
    dt_type = sa.DateTime() if dialect == "sqlite" else sa.DateTime(timezone=True)
    op.create_table("manual_activation_requests", sa.Column("id", sa.String(100), primary_key=True), sa.Column("user_id", sa.String(100), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False), sa.Column("requested_tier", sa.String(50), nullable=False, server_default=sa.text("'pro'")), sa.Column("proof_url", sa.String(1024), nullable=True), sa.Column("notes", sa.Text(), nullable=True), sa.Column("admin_notes", sa.Text(), nullable=True), sa.Column("status", sa.String(20), nullable=False, server_default=sa.text("'pending'")), sa.Column("reviewed_by", sa.String(100), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True), sa.Column("reviewed_at", dt_type, nullable=True), sa.Column("created_at", dt_type, nullable=False, server_default=sa.func.now()))
