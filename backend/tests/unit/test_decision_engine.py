...
def make_attendee(timezone_str: str="UTC", avg_response_time_hours: float=2.0) -> AttendeeAnalysis:
    return AttendeeAnalysis(email="attendee@example.com", vip_level=VIPLevel.STANDARD, is_new=False, booking_frequency=2, no_show_rate=0.0, avg_response_time_hours=avg_response_time_hours, preferred_communication=["email"], engagement_score=0.5, timezone=timezone_str)
...
class TestTimingAnalysisDataclass:
    ...
    def test_timezone_offset_hours_is_int(self):
        ...
        analysis = TimingAnalysis(optimal_send_time=datetime.now(UTC).isoformat(), timezone_offset_hours=0, expected_response_time_hours=1.0, urgency_level="medium", business_hours_aligned=True)
    ...
    def test_timezone_offset_hours_positive_int(self):
        ...
        analysis = TimingAnalysis(optimal_send_time=datetime.now(UTC).isoformat(), timezone_offset_hours=5, expected_response_time_hours=1.0, urgency_level="medium", business_hours_aligned=True)
    ...
    def test_timezone_offset_hours_negative_int(self):
        ...
        analysis = TimingAnalysis(optimal_send_time=datetime.now(UTC).isoformat(), timezone_offset_hours=-8, expected_response_time_hours=2.0, urgency_level="low", business_hours_aligned=False)
...
class TestAnalyzeTiming:
    ...
    async def test_non_utc_timezone_returns_correct_offset(self, engine):
        ...
        test_cases = [("Asia/Kolkata", 5), ("UTC", 0)]
