...
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", "15"))  # Reduced from 1 week to 15 minutes
REFRESH_TOKEN_EXPIRE_DAYS = int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS", "7"))  # Reduced from 30 days to 7 days
...
def validate_secret_key() -> str:
    ...
    if is_production:
        if not raw_secret:
            ...
            print("CRITICAL SECURITY ERROR: SECRET_KEY environment variable is not set!", file=sys.stderr)
            print("=" * 80, file=sys.stderr)
            print("\nJWT authentication is disabled without a valid SECRET_KEY.", file=sys.stderr)
            ...
            print("2. Set the SECRET_KEY environment variable in your production environment", file=sys.stderr)
        ...
        if raw_secret.lower() in {k.lower() for k in DEFAULT_KEYS}:
            print("=" * 80, file=sys.stderr)
            print("CRITICAL SECURITY ERROR: SECRET_KEY is set to a default/weak value!", file=sys.stderr)
            print("=" * 80, file=sys.stderr)
        ...
        if len(raw_secret) < 32:
            ...
            print("Required: Minimum 32 characters (256 bits of entropy)", file=sys.stderr)
        ...
        if len(raw_secret) < 43:
            print("WARNING: SECRET_KEY is shorter than recommended 43 characters", file=sys.stderr)
            print("Consider generating a new key for enhanced security:", file=sys.stderr)
            print("   python -c 'import secrets; print(secrets.token_urlsafe(32))'", file=sys.stderr)
    ...
    # Development: Generate temporary key with warning
        ...
        print("=" * 80, file=sys.stderr)
        print("\nThis secret is temporary and will change on each restart.", file=sys.stderr)
