...
async def test_sync_calendar_token_logs_completed_audit_event():
    ...
    with patch("backend.services.sync_engine.get_calendar_provider_for_token", return_value=provider), patch("backend.services.sync_engine.log_activity", new=AsyncMock()) as mock_log_activity:
...
async def test_sync_calendar_token_logs_skipped_provider_audit_event():
    ...
    with patch("backend.services.sync_engine.get_calendar_provider_for_token", return_value=None), patch("backend.services.sync_engine.log_activity", new=AsyncMock()) as mock_log_activity:
        await sync_calendar_token(db, token)
    mock_log_activity.assert_awaited_once()
    kwargs = mock_log_activity.await_args.kwargs
