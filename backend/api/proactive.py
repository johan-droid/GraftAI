...
async def suggest_action(payload: ProactiveSuggestRequest, current_user: UserTable=Depends(get_current_user)):
    ...
    if "dashboard" in context:
        ...
        smart_actions = [SmartAction(id="action_dash_1", action_type="review_schedule", title="Review Schedule", description="Look at today's upcoming meetings.", confidence_score=0.9)]
    elif "meeting" in context:
        suggestion = "Ask me to find the best time to schedule a follow-up meeting."
        smart_actions = [SmartAction(id="action_meet_1", action_type="schedule", title="Schedule Follow-up", description="Find time for a follow-up meeting.", confidence_score=0.8, payload={"meeting_type": "follow_up"})]
    else:
        suggestion = "Try asking for your next availability or to summarize today’s schedule."
        smart_actions = [SmartAction(id="action_gen_1", action_type="summarize", title="Summarize Day", description="Get a quick summary of your schedule today.", confidence_score=0.75)]
