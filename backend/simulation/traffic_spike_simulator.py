...
class TrafficSpikeSimulator:
    ...
    async def simulate_traffic_spike(self, pattern: TrafficPattern) -> dict[str, Any]:
        ...
        logger.info("Starting traffic spike simulation: %s", pattern.name)
        logger.info("Baseline: %s RPS, Spike: %sx", pattern.baseline_rps, pattern.spike_multiplier)
        ...
        return await self._generate_simulation_report(pattern)

    async def _measure_baseline(self, rps: int, duration_seconds: int):
        """Measure baseline performance"""
        logger.info("Measuring baseline at %s RPS for %ss", rps, duration_seconds)
        tasks = []
        for _ in range(rps):
    ...
    async def _ramp_up_traffic(self, pattern: TrafficPattern):
        ...
        for step in range(ramp_steps):
            ...
            logger.info("Ramp up step %s/%s: %s RPS", step + 1, ramp_steps, current_rps)
            tasks = []
    ...
    async def _maintain_peak_load(self, pattern: TrafficPattern):
        ...
        for second in range(pattern.spike_duration_seconds):
            logger.info("Peak load second %s/%s: %s RPS", second + 1, pattern.spike_duration_seconds, target_rps)
            tasks = []
    ...
    async def _ramp_down_traffic(self, pattern: TrafficPattern):
        ...
        for step in range(ramp_steps):
            ...
            logger.info("Ramp down step %s/%s: %s RPS", step + 1, ramp_steps, current_rps)
            tasks = []
    ...
    async def _check_failure_conditions(self, timestamp: datetime):
        ...
        if self.resource_usage.get("cpu", []) and self.resource_usage["cpu"][-1] > 90:
            failure = FailurePoint(component="CPU", failure_type=FailureType.CPU_EXHAUSTION, threshold_reached=self.resource_usage["cpu"][-1], time_to_failure=(timestamp - self.simulation_start).total_seconds(), impact_severity="critical", symptoms=["High response times", "Request timeouts", "System unresponsiveness"])
            self.failure_points.append(failure)
        if self.resource_usage.get("memory", []) and self.resource_usage["memory"][-1] > 85:
            failure = FailurePoint(component="Memory", failure_type=FailureType.MEMORY_EXHAUSTION, threshold_reached=self.resource_usage["memory"][-1], time_to_failure=(timestamp - self.simulation_start).total_seconds(), impact_severity="critical", symptoms=["Out of memory errors", "Process crashes", "Swap thrashing"])
            self.failure_points.append(failure)
        total_requests = self.successful_requests + self.failed_requests
        if total_requests > 100:
            error_rate = self.failed_requests / total_requests
            if error_rate > 0.1:
                failure = FailurePoint(component="API", failure_type=FailureType.SERVICE_DEGRADATION, threshold_reached=error_rate * 100, time_to_failure=(timestamp - self.simulation_start).total_seconds(), impact_severity="high", symptoms=["HTTP 5xx errors", "Request timeouts", "Connection refused"])
                self.failure_points.append(failure)
        if len(self.response_times) > 10:
            avg_response_time = statistics.mean(self.response_times[-10:])
            if avg_response_time > 5.0:
                failure = FailurePoint(component="API", failure_type=FailureType.API_TIMEOUT, threshold_reached=avg_response_time, time_to_failure=(timestamp - self.simulation_start).total_seconds(), impact_severity="medium", symptoms=["Slow responses", "User experience degradation"])
                self.failure_points.append(failure)
    ...
    def _check_critical_failures(self) -> bool:
        ...
        if self.resource_usage.get("memory", []) and self.resource_usage["memory"][-1] > 95:
            return True
        total_requests = self.successful_requests + self.failed_requests
        return bool(total_requests > 50 and self.failed_requests / total_requests > 0.5)
    ...
    async def _generate_simulation_report(self, pattern: TrafficPattern) -> dict[str, Any]:
        ...
        return {"simulation_summary": {"pattern_name": pattern.name, "baseline_rps": pattern.baseline_rps, "peak_rps": int(pattern.baseline_rps * pattern.spike_multiplier), "spike_duration": pattern.spike_duration_seconds, "total_requests": self.successful_requests + self.failed_requests, "success_rate": self.successful_requests / (self.successful_requests + self.failed_requests) if self.successful_requests + self.failed_requests > 0 else 0, "avg_response_time": statistics.mean(self.response_times) if self.response_times else 0, "max_response_time": max(self.response_times) if self.response_times else 0, "simulation_duration": (datetime.now(UTC) - self.simulation_start).total_seconds()}, "failure_points": [asdict(fp) for fp in self.failure_points], "bottlenecks": [asdict(b) for b in self.bottlenecks], "data_consistency_issues": [asdict(dci) for dci in self.consistency_issues], "resource_usage": {"cpu": {"max": max(self.resource_usage.get("cpu", [0])), "avg": statistics.mean(self.resource_usage.get("cpu", [0])), "final": self.resource_usage.get("cpu", [0])[-1] if self.resource_usage.get("cpu") else 0}, "memory": {"max": max(self.resource_usage.get("memory", [0])), "avg": statistics.mean(self.resource_usage.get("memory", [0])), "final": self.resource_usage.get("memory", [0])[-1] if self.resource_usage.get("memory") else 0}}, "recommendations": self._generate_recommendations()}
    ...
    async def _analyze_bottlenecks(self):
        ...
        if self.resource_usage.get("cpu", []):
            ...
            if max_cpu > 80:
                bottleneck = BottleneckReport(bottleneck_type=BottleneckType.CPU_UTILIZATION, component="CPU", baseline_value=statistics.mean(cpu_usage[:10]) if len(cpu_usage) > 10 else cpu_usage[0], peak_value=max_cpu, time_to_peak=cpu_usage.index(max_cpu), duration_above_threshold=len([x for x in cpu_usage if x > 80]), impact_on_users="High response times, request timeouts", recommended_fixes=["Scale up CPU resources", "Optimize CPU-intensive operations", "Implement request queuing"])
                self.bottlenecks.append(bottleneck)
        if self.resource_usage.get("memory", []):
            memory_usage = self.resource_usage["memory"]
            max_memory = max(memory_usage)
            if max_memory > 75:
                bottleneck = BottleneckReport(bottleneck_type=BottleneckType.MEMORY_ALLOCATION, component="Memory", baseline_value=statistics.mean(memory_usage[:10]) if len(memory_usage) > 10 else memory_usage[0], peak_value=max_memory, time_to_peak=memory_usage.index(max_memory), duration_above_threshold=len([x for x in memory_usage if x > 75]), impact_on_users="Out of memory errors, process crashes", recommended_fixes=["Increase memory allocation", "Optimize memory usage", "Implement memory caching"])
                self.bottlenecks.append(bottleneck)
        if self.response_times:
            ...
            if max_avg_response > 2.0:
                bottleneck = BottleneckReport(bottleneck_type=BottleneckType.API_RESPONSE_TIME, component="API", baseline_value=avg_response_times[0] if avg_response_times else 0, peak_value=max_avg_response, time_to_peak=avg_response_times.index(max_avg_response), duration_above_threshold=len([x for x in avg_response_times if x > 2.0]), impact_on_users="Poor user experience, abandoned requests", recommended_fixes=["Add API caching", "Optimize database queries", "Implement request timeouts"])
                self.bottlenecks.append(bottleneck)

    async def _check_data_consistency(self):
        """Check for data consistency issues"""
        if self.failed_requests > 100:
            issue = DataConsistencyIssue(entity_type="booking", inconsistency_type="duplicate_bookings", affected_records=int(self.failed_requests * 0.1), detection_time=datetime.now(UTC), root_cause="Race conditions during high load", severity="medium", auto_recovery_possible=True)
            self.consistency_issues.append(issue)
        if max(self.resource_usage.get("cpu", [0])) > 90:
            issue = DataConsistencyIssue(entity_type="user", inconsistency_type="quota_inconsistency", affected_records=int(self.successful_requests * 0.05), detection_time=datetime.now(UTC), root_cause="Concurrent quota updates without proper locking", severity="high", auto_recovery_possible=False)
            self.consistency_issues.append(issue)
...
TRAFFIC_PATTERNS = {"moderate_spike": TrafficPattern(name="Moderate Spike", baseline_rps=50, spike_multiplier=5.0, spike_duration_seconds=60, ramp_up_seconds=30, ramp_down_seconds=30, endpoints=["/health", "/api/v1/users/me", "/api/v1/bookings"], user_distribution={"free": 0.7, "pro": 0.25, "elite": 0.05}), "severe_spike": TrafficPattern(name="Severe Spike", baseline_rps=100, spike_multiplier=10.0, spike_duration_seconds=120, ramp_up_seconds=15, ramp_down_seconds=30, endpoints=["/health", "/api/v1/users/me", "/api/v1/bookings", "/api/v1/ai/chat"], user_distribution={"free": 0.6, "pro": 0.3, "elite": 0.1}), "extreme_spike": TrafficPattern(name="Extreme Spike", baseline_rps=200, spike_multiplier=20.0, spike_duration_seconds=180, ramp_up_seconds=10, ramp_down_seconds=20, endpoints=["/health", "/api/v1/users/me", "/api/v1/bookings", "/api/v1/ai/chat", "/api/v1/calendar/sync"], user_distribution={"free": 0.5, "pro": 0.35, "elite": 0.15}), "flash_crowd": TrafficPattern(name="Flash Crowd", baseline_rps=10, spike_multiplier=50.0, spike_duration_seconds=30, ramp_up_seconds=5, ramp_down_seconds=10, endpoints=["/api/v1/bookings/create", "/api/v1/ai/chat"], user_distribution={"free": 0.8, "pro": 0.15, "elite": 0.05})}
...
async def run_traffic_spike_simulation(pattern_name: str="moderate_spike") -> dict[str, Any]:
    ...
    return await simulator.simulate_traffic_spike(pattern)
