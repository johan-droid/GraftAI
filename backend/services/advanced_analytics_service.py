...
class AdvancedAnalyticsService:
    ...
    async def get_team_metrics(
        ...
        total_revenue = sum([
            b.metadata.get("revenue", 0) for b in bookings 
        ...
        return {
            ...
            "bookings": {
                ...
                "cancelled": cancelled,
                "pending": pending,
                "completion_rate": round(confirmed / total_bookings * 100, 2) if total_bookings else 0,
    ...
    async def get_booking_trends(
        ...
        stmt = select(
            ...
            func.count(func.case([(TeamBooking.status == "confirmed", 1)])).label("confirmed"),
            func.count(func.case([(TeamBooking.status == "cancelled", 1)])).label("cancelled")
        ).where(
            and_(
                TeamBooking.team_id == team_id,
                TeamBooking.created_at >= start_date,
            ...
            func.date(TeamBooking.created_at)
    ...
    async def get_member_performance(
        ...
        # Get all members
        stmt = select(TeamMember, UserTable).join(
            UserTable, TeamMember.user_id == UserTable.id
        ...
        for member, user in members:
            ...
            performance.append({
                ...
                "metrics": {
                    ...
                    "completion_rate": round(len(confirmed) / len(bookings) * 100, 2) if bookings else 0,
                    "total_duration_hours": sum([
                        (b.end_time - b.start_time).total_seconds() / 3600
                        for b in confirmed if b.end_time and b.start_time
                    ])
                }
            })
    ...
    async def get_revenue_analytics(
        ...
        return {
            ...
            "summary": {
                ...
                "avg_revenue_per_booking": round(float(result.total_revenue or 0) / result.total_bookings, 2) if result.total_bookings else 0
            },
            "daily_trend": [
                {
                    "date": str(row.date),
    ...
    async def get_peak_hours_analysis(
        ...
        stmt = select(
            ...
            extract('hour', TeamBooking.start_time)
        ...
        return {
            ...
            "hourly_distribution": [
                {"hour": h, "bookings": c, "period": f"{h:02d}:00-{(h+1)%24:02d}:00"}
    ...
    async def get_dashboard_summary(
        ...
        if team_id:
            ...
            ).order_by(TeamBooking.start_time).limit(5)
