    ...
    def _load_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_risk": {
                ...
                "actions": [
                    {"type": "send_email", "template": "high_risk_confirmation", "priority": "critical"},
                    {"type": "send_sms", "template": "reminder", "priority": "high"},
                    {"type": "create_calendar_event", "priority": "high"},
                    {"type": "create_task", "task_type": "monitoring", "priority": "high"},
                ],
            ...
            "vip": {
                ...
                "actions": [
                    {"type": "send_email", "template": "vip_welcome", "priority": "critical"},
                    {"type": "send_sms", "template": "vip_confirmation", "priority": "high"},
                    {"type": "create_calendar_event", "priority": "critical"},
                    {"type": "create_task", "task_type": "vip_followup", "priority": "critical"},
                    {"type": "post_to_slack", "channel": "#vip-bookings", "priority": "high"},
                ],
            ...
            "critical_risk": {
                ...
                "actions": [
                    {"type": "send_email", "template": "critical_confirmation", "priority": "critical"},
                    {"type": "send_sms", "template": "urgent_reminder", "priority": "critical"},
                    {"type": "create_calendar_event", "priority": "critical"},
                    {"type": "create_task", "task_type": "critical_monitoring", "priority": "critical"},
                    {"type": "post_to_slack", "channel": "#critical-bookings", "priority": "critical"},
                ],
            ...
            "standard": {
                "conditions": {},
                "actions": [
                    {"type": "send_email", "template": "standard_confirmation", "priority": "medium"},
...
class AdminNotifier:
    ...
    async def notify(
        ...
        """
        logger.warning(f"🔔 Admin notification: Booking {booking_id} needs review - {reason}")
...
class FallbackOrchestrator:
    ...
    async def automate_with_fallback(
        ...
        try:
            ...
            logger.info(f"[{booking_id}] ✅ AI Agent succeeded: score={result.decision_score}")
        ...
        try:
            ...
            if attendee:
                ...
                if result.status in ["completed", "partial"]:
                    logger.info(f"[{booking_id}] ✅ Rule-based fallback succeeded: {result.status}")
                    return result
                else:
                    logger.warning(f"[{booking_id}] Rule-based fallback returned: {result.status}")
            else:
...
async def notify_admin(message: str, booking_id: Optional[str] = None) -> Dict[str, Any]:
    """
