...
class BookingAutomationService:
    ...
    async def _remember_booking(
        ...
        episode = {
            ...
            if any(r.get("success") for r in action_results)
            else "failure",
            "result": {
    ...
    async def _recall_similar_bookings(
        self, attendee_email: str, organizer_id: str
    ) -> List[Dict[str, Any]]:
        ...
        results = await self.memory_manager.long_term.retrieve_similar_episodes(
            query, 5
        )
    ...
    async def process_booking_created(
        ...
        try:
            ...
            self.memory_manager = await create_memory_manager(
                user_id=user_id, vector_store=self.vector_store
            )
            ...
            logger.info(
                f"  - Past bookings: {perception_data['attendee_info'].get('past_bookings', 0)}"
            )
            logger.info(
                f"  - No-show rate: {perception_data['attendee_info'].get('no_show_rate', 0):.1%}"
            )
            ...
            log_agent_decision(
                ...
                actions=[
                    {"tool_name": a.tool_name, "priority": a.priority.name}
            ...
            for result in action_results:
                ...
                logger.info(
                    f"  {status} {result.get('tool_name')}: {result.get('status', 'unknown')}"
                )
            ...
            external_results = {
                ...
                "calendar_id": next(
                    (
                        r.get("event_id")
                        for r in action_results
                        if r.get("tool_name") == "create_calendar_event"
                    ),
                    None,
                ),
                "task_id": next(
                    (
                        r.get("task_id")
                        for r in action_results
                        if r.get("tool_name") == "create_task"
                    ),
                    None,
                ),
            }
            ...
            metrics.record_error(
                error_type="automation_failure", component="booking_automation"
            )
            metrics.record_automation_complete(
                booking_id=booking_id,
                status="failed",
        ...
        async with session_factory() as db:
            ...
            if booking:
                ...
                booking_data = {
                    ...
                    or f"Booking with {booking.full_name}",
            ...
            recent_interactions = await self.memory_manager.retrieve_by_context(
                query=f"interactions with {booking_data['attendee_email']}", n_results=5
            )

            # 3b. Recall similar past booking outcomes
            similar_bookings = await self._recall_similar_bookings(
                booking_data["attendee_email"], booking_data["organizer_id"]
            )
            ...
            context = {
                ...
                "lead_time_hours": self._calculate_lead_time(
    ...
    async def _action_phase(
        ...
        for action_id in decision.execution_order:
            ...
            logger.info(
                f"  Executing: {action.tool_name} (Priority: {action.priority.value})"
            )
            ...
            try:
                ...
                await self.memory_manager.context.store_phase_result(
                    phase="action", key=f"tool_{action.tool_name}", value=result
                )
    ...
    async def _reflection_phase(
        ...
        email_result = next(
            (r for r in action_results if r.get("tool_name") == "send_email"), None
        )
        ...
        await self._remember_booking(
            ...
            action_results=action_results,
            success=all_success,
            decision_score=decision.confidence.value
    ...
    async def _store_results_with_transaction(
        ...
        try:
            ...
            agent_decisions = {
                ...
                "reasoning": decision.reasoning,
                "confidence": getattr(
                    decision.confidence, "name", str(decision.confidence)
                ),
                "risk_assessment": decision.risk_analysis.level.value,
    ...
    def _calculate_decision_score(
        ...
        success_rate = (
            sum(1 for r in action_results if r.get("success")) / len(action_results)
            if action_results
            else 0
        )
...
def on_booking_created(booking_id: str, user_id: str) -> AutomationResult:
    ...
    # Run async workflow
    return asyncio.run(
        service.process_booking_created(
            booking_id=booking_id, user_id=user_id, trigger_source="scheduler_webhook"
        )
    )
