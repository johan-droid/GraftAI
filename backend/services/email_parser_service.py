...
class EmailParserService:
    ...
    # Meeting keywords
    MEETING_KEYWORDS = [
        'meet', 'meeting', 'sync', 'call', 'discussion', 'chat', 'talk',
        'conference', 'review', 'standup', 'stand-up', '1:1', 'one-on-one',
        'interview', 'presentation', 'demo', 'workshop', 'brainstorm',
    ]
    
    CONFIRMATION_KEYWORDS = [
        'confirmed', 'accepted', 'yes', 'ill be there', 'see you',
        'works for me', 'sounds good', 'im in', 'count me in',
    ]
    
    CANCELLATION_KEYWORDS = [
        'cancel', 'cancelled', 'cant make it', 'wont be able',
        'need to reschedule', 'something came up', 'unavailable',
    ]
    
    RESCHEDULE_KEYWORDS = [
        'reschedule', 'move', 'change', 'different time', 'another time',
        'wont work', 'conflict', 'overlap',
    ]
    ...
    def _extract_title(self, subject: str, body: str) -> Optional[str]:
        ...
        prefixes_to_remove = [
            r'^re:\s*', r'^fw[d]?:\s*', r'^fwd:\s*',
            r'^meeting:\s*', r'^scheduled:\s*',
        ]
    ...
    def _extract_datetime(self, text: str, reference_time: datetime) -> Optional[datetime]:
    ...
    def _calculate_confidence(
        self, intent: EmailIntent, title: Optional[str],
        start_time: Optional[datetime], location: Optional[str]
    ) -> float:
