...
@register_tool(
    ...
    examples=[{"user_id": "user_123", "timeframe_days": 90}],
)
...
@register_tool(
    ...
    examples=[{"booking_id": "booking_123", "attendee_history": []}],
)
async def predict_no_show_risk(
    booking_id: str, attendee_history: Optional[List[dict]] = None
) -> dict:
    """
...
async def find_best_time_slot(
    attendees: List[str], duration_minutes: int, constraints: Optional[Dict] = None
) -> dict:
    """
    ...
    try:
        logger.info(
            f"Finding best slot for {len(attendees)} attendees, {duration_minutes} min"
        )
...
async def get_attendee_preferences(attendee_id: str) -> dict:
    ...
    try:
        ...
        preferences = {
            ...
            "auto_accept_threshold": {"duration_minutes": 30, "attendees_max": 5},
