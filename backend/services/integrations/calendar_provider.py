...
class CalendarSyncProvider(ABC):
    ...
    async def sync(self, db: AsyncSession) -> int:
        ...
        if not access_token:
            logger.warning("%s sync skipped: invalid or expired token for user %s", self.name, self.token_record.user_id)
            return 0
        ...
        for item in events:
            ...
            if normalized.get("removed"):
                ...
                if ext_id:
                    stmt = select(EventTable).where(and_(EventTable.user_id == self.token_record.user_id, EventTable.is_deleted.is_(False), EventTable.external_id == ext_id))
...
class GoogleCalendarSyncProvider(CalendarSyncProvider):
    ...
    async def fetch_events(self, access_token: str, sync_token: str | None) -> tuple[list[dict[str, Any]], str | None]:
        ...
        if refresh_token is None:
            logger.error("Cannot fetch Google events. Token decryption failed for token ID %s", self.token_record.id)
            msg = f"Token decryption failed for token ID {self.token_record.id}"
    ...
    async def get_busy_windows(self, db: AsyncSession, start: datetime, end: datetime) -> list[dict[str, Any]]:
        refresh_token, _ = decrypt_token_value(self.token_record.refresh_token)
        if refresh_token is None:
            logger.error("Cannot get Google busy windows. Token decryption failed for token ID %s", self.token_record.id)
            msg = f"Token decryption failed for token ID {self.token_record.id}"
            raise ValueError(msg)
        token_data = {"access_token": await ensure_valid_token(db, self.token_record.user_id, "google"), "refresh_token": refresh_token}
    ...
    def normalize_event(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"external_id": item.get("id"), "title": item.get("summary", "Untitled Event"), "description": item.get("description"), "location": item.get("location"), "start_time": self._parse_datetime(start_str), "end_time": self._parse_datetime(end_str), "source": "google", "meeting_url": item.get("hangoutLink") or item.get("htmlLink"), "attendees": [attendee.get("email") for attendee in item.get("attendees", []) if attendee.get("email")]}

    async def create_event(self, event_details: dict[str, Any]) -> dict[str, Any]:
        refresh_token, _ = decrypt_token_value(self.token_record.refresh_token)
        access_token, _ = decrypt_token_value(self.token_record.access_token)
        if access_token is None:
            msg = f"Cannot create Google event. Token decryption failed for token ID {self.token_record.id}"
            raise ValueError(msg)
        token_data = {"access_token": access_token, "refresh_token": refresh_token}
    ...
    async def delete_event(self, external_event_id: str) -> bool:
        ...
        access_token, _ = decrypt_token_value(self.token_record.access_token)
        if access_token is None:
            msg = f"Cannot delete Google event. Token decryption failed for token ID {self.token_record.id}"
            raise ValueError(msg)
        token_data = {"access_token": access_token, "refresh_token": refresh_token}
...
class MicrosoftGraphSyncProvider(CalendarSyncProvider):
    ...
    def normalize_event(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"external_id": item.get("id"), "title": item.get("subject", "Untitled Event"), "description": item.get("bodyPreview"), "location": item.get("location", {}).get("displayName"), "start_time": start_time, "end_time": end_time, "source": "microsoft", "meeting_url": item.get("onlineMeeting", {}).get("joinUrl") or item.get("webLink"), "attendees": [recipient.get("emailAddress", {}).get("address") for recipient in item.get("attendees", []) if recipient.get("emailAddress")]}

    async def create_event(self, event_details: dict[str, Any]) -> dict[str, Any]:
        refresh_token, _ = decrypt_token_value(self.token_record.refresh_token)
        token_data = {"refresh_token": refresh_token, "scopes": self.token_record.scopes}
...
def get_calendar_provider_for_token(token_record: UserTokenTable) -> CalendarSyncProvider | None:
    ...
    logger.warning("Unsupported calendar provider: %s", token_record.provider)
    return None
